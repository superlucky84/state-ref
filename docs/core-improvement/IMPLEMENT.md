...
## Phase 7 — 테스트 하드닝  ✅ 완료 (2026-09-18) / ⚠️ `CI-23`·`CI-24` 유입 아닌 **발견**, 같은 Phase에서 수정

**진입 조건:** Phase 1~6 전부 종료 ✅ (Phase 6.5 포함, `dbdc835`)

> **계획 정정 (2026-09-18).** 착수 시점에 체크리스트 세 항목이 현행 코드와 어긋나 있었다. 계획은 Phase 0~2 시점의 코드를 가정하고 쓰였다.
> - **`batch sync|microtask` 축 삭제** — `DC-03`이 배칭 미도입으로 종결되고 `INV-4`(알림은 동기 1회)로 승격됐다. 존재하지 않는 축이다.
> - **`keyIndex.size === 0` → 경로 트리 기준으로 교체** — `DC-10`이 문자열 key 인덱스를 `PathNode` 트리로 대체해 `keyIndex`는 더 이상 없다. 누수의 관측 지점은 `storeRenderList`와 노드의 `subs`다.
> - **"CI에 편입" → `pnpm gate` 정의** — `DC-15`. 이 저장소에 CI가 없다.

**체크리스트**
- [ ] Phase 0 회귀 스냅샷(`src/tests/core/regression.ts`, 25건)을 최종 동작 기준으로 갱신, 각 변경에 CI-ID 주석 부착
- [ ] 엣지 케이스 매트릭스 작성 및 테스트화
      - 값 타입: primitive / object / array / null / undefined / Symbol 키 / 중첩 배열
      - 모드: autoSync / manualSync × editable true|false × cache true|false × trackDeps true|false
- [x] 재진입 테스트 — 구독 콜백 안에서 `.value` 쓰기. 무한 루프 방지 가드 및 최대 깊이 정의 → **`CI-23` 발견, `DC-16`으로 해소 (깊이 100 + throw)**
- [x] 구독 해제 경쟁 조건 — `runner` 실행 중 `abort()` 호출 → **`CI-24` 발견, `DC-17`로 해소 (해제된 `run` 건너뛰기)**
- [x] 동일 틱에 같은 값 재대입(`value === current`) 시 알림 미발생 확인 — autoSync·manualSync 양쪽 통과, 결함 없음
- [x] 누수 테스트 — 1,000회 구독/해제 반복 후 `storeRenderList.size === 0`, **그리고 경로 트리의 모든 노드에서 `subs`가 비어 있음**(`create`가 반환하는 `pathRoot` seam으로 관측). 노드 자체가 남는 것은 `DC-14` 이후에도 설계된 동작이므로 노드 수는 **상한**으로만 본다
- [ ] 깊이 64 경로 스트레스
- [ ] 1만 노드 배열 스트레스
- [x] **`pnpm gate` 정의** (`DC-15`) — 빌드 → `tsc --noEmit` → eslint → `pnpm test` → 벤치 게이트(NFR-1/NFR-2/ACCUMULATION) → 번들 상한(NFR-3) 순서, 첫 실패에서 비영 종료. 벤치가 `dist`를 읽으므로 **빌드가 선행**해야 한다
- [x] 게이트 실패 주입 검증 3종 — 타입 오류 / 번들 초과 / 벤치 회귀를 각각 하나씩 넣어 비영 종료 확인 후 되돌림
- [ ] `docs/core-improvement/`의 재현 스크립트가 최신 소스에서 동작하는지 확인

### 게이트 (`DC-15`) — 완료 (2026-09-18)

`scripts/gate.mjs` + 루트 `pnpm gate`. 빌드 → 타입 → 린트 → 테스트 → 벤치 → 번들 순서, 첫 실패에서 비영 종료. `--quick`은 빌드만 건너뛰며, `dist`가 없으면 `exit 2`로 거부한다(낡은 산출물을 재는 조용한 통과를 막는다).

**실패 주입 3종 — 전부 막혔다**

| 주입 | 방법 | 결과 |
|---|---|---|
| 타입 오류 | `path/index.ts`에 `const x: number = "..."` | `types` 단계 FAIL, 게이트 중단 |
| 번들 초과 | `dist`에 비압축 3.2 KB 추가 | `bundle` 5,674 B > 3,200 B FAIL, 게이트 중단 |
| 벤치 회귀 | `dist`의 프록시 `get` 트랩에 루프 주입 | 깊이 8이 11.9 → 717.8 ms, 게이트 6/6 → **5/6** FAIL |

주입은 모두 되돌렸고 워킹트리에 남지 않았다.

**발견 두 가지**

1. **`childOf`는 읽기 핫패스가 아니다.** 벤치 회귀를 처음에 `childOf`에 주입했는데 깊이 8이 11.9 ms 그대로였다 — 6.5-B 이후 구독이 닿지 않는 읽기는 노드를 만들지 않으므로 `childOf`를 **아예 호출하지 않는다.** 설계가 의도한 대로 동작한다는 독립적인 확인이다. 읽기 비용은 프록시 `get` 트랩에 있다.
2. **번들 수치가 기록과 다르다.** `bench-phase6-5.txt:32`는 minified gzip **3,197 B**, 지금 재면 **3,173 B**다. raw는 7,774 B로 **양쪽이 같다** — 같은 길이인데 gzip만 24 B 다르다. 상한(3,200 B) 아래이므로 진행을 막지 않지만, `DC-09`의 잔여는 문서가 말하는 8 B가 아니라 **27 B**다. Phase 7 종료 시 재측정해 확정한다.

### 전파 패스의 경계 — 완료 (2026-09-18)

`src/tests/core/hardening.ts` 12건 신규 (코어 157 → **169**). 집 규칙대로 **실패하는 테스트를 먼저** 두고 고쳤다 — 3건이 빨간 상태에서 시작해 전부 초록이 됐다.

**결함 두 개를 찾았다. 둘 다 출시된 2.1.0에 있고, 이 브랜치가 들여온 것이 아니다.**

| | `CI-23` 재귀 상한 없음 | `CI-24` 해제가 되살아남 |
|---|---|---|
| 증상 | 자기를 먹이는 구독자가 스택을 터뜨리고, `RangeError`를 리포터가 삼키고, 그 `console.error`가 다시 오버플로 | 패스 중 발화한 `AbortSignal`이 `removeRun`으로 지워진 뒤, 같은 패스에서 그 구독자가 실행되며 경로를 읽어 **재등록** |
| 사용자가 보는 것 | 중간 값(`n = 956`)에 멈춘 스토어 + 진단 불가능한 콘솔 잔해 | **영구 구독.** signal은 이미 발화해 다시 해제할 수단이 없다 |
| 결정 | `DC-16` — 깊이 100 + throw | `DC-17` — 해제된 `run`을 그 패스에서 건너뛴다 |
| 비용 | +143 B | +31 B |

정상이던 3가지도 함께 고정했다 — 연쇄 재진입(A→B→C), `false` 반환 해제, 패스 밖 `abort()`. 그리고 **깊이 카운터가 패스 사이에 누수되지 않는지**(얕은 쓰기 300회가 상한을 건드리지 않는다)를 별도로 고정했다. `try/finally`가 없으면 구독자가 던진 순간 깊이가 영영 돌아오지 않는다.

**번들 — `DC-09` 3차 상향.** 3,173 → **3,347 B**, 상한 3,200 → **3,400 B**.

**측정 자체의 결함도 하나 찾았다.** 같은 산출물의 gzip 수치가 게이트에서 3,320 B, 셸에서 3,347 B로 달랐다. 원인은 **Node 버전**이다 — `pnpm exec node`는 22.13.0, 볼타가 핀한 `node`는 20.3.0이고 zlib이 다르다. **27 B는 이 예산에서 변경 하나의 크기**라 그냥 둘 수 없었다. `bundle-size.mjs`가 측정한 Node 버전을 함께 출력하고, 상한은 핀된 20.3.0 기준으로 정의한다.

### CI-01 ~ CI-24 추적표 (2026-09-18)

| CI | 결말 | 검증 |
|---|---|---|
| CI-01 | Phase 1 수정 | `regression.ts` — manual-sync에서 `{cache:false}`도 쓰기를 막는다 (4건) |
| CI-02 | Phase 2 수정 | `regression.ts` — `JSON.stringify` / `Object.keys` / `in` / 스프레드 (6건) |
| CI-03 | Phase 2 수정 | `regression.ts` — `delete` 거부, 원시형 강제변환 메시지, `valueOf`/`toString` (3건) |
| CI-04 | Phase 6 수정 (`DC-01`) | `clone-and-paths.ts`, `regression.ts`, `stress.ts`(깊이 64에서 끊긴 경로) |
| CI-05 | Phase 1 수정 | `regression.ts` — 던지는 구독자 뒤의 구독자도 실행, 집계 보고 (3건) |
| CI-06 | Phase 5 수정 | `lifecycle.ts`, `regression.ts`, `connect-react/unmount-leak.tsx` |
| CI-07 | Phase 5 수정 (`DC-11`) | `lifecycle.ts`, `regression.ts`, `connect-react/unmount-leak.tsx` |
| CI-08 | Phase 6 수정 (`DC-07`) | `clone-and-paths.ts` — Symbol 키·Date/Map/Set/RegExp·순환참조 (26건 중) |
| CI-09 | `DC-10`으로 원인 소멸 | `regression.ts` — Symbol 경로에 전역 레지스트리가 필요 없다 |
| CI-10 | Phase 2 수정 (`DC-04`) | `regression.ts` — 타입/런타임 일치, `length`가 반응 경로 |
| CI-11 | Phase 3에서 제거 | **벤치 `NFR-1` 게이트** (깊이 2/8/32 읽기). 단위 테스트로 잴 수 있는 성질이 아니다 — 아래 주 참조 |
| CI-12 | Phase 3 수정 | `regression.ts` — 무관한 경로는 읽히지도 않는다 (5건) |
| CI-13 | **비목표** (`DC-03`, `INV-4`) | `regression.ts` — 대입 2회 = 콜백 2회가 **정의된 동작**임을 고정 |
| CI-14 | Phase 5 opt-in (`DC-02`) | `lifecycle.ts`(opt-in 쪽), `regression.ts`(기본값 쪽) |
| CI-15 | Phase 3 수정 | `regression.ts`, `tree-lifetime.ts`, `matrix.ts`(16조합 전부에서 identity 안정) |
| CI-16 | Phase 5 부분 수정 | `lifecycle.ts`, `regression.ts`, `matrix.ts`(16조합 전부에서 `cache` 계약) |
| CI-17 | Phase 5 확정 (`DC-06`) | `lifecycle.ts`, `hardening.ts`(1,000회 구독/해제 후 잔여 0) |
| CI-18 | 정정 (도달 가능) | `regression.ts` — 던지는 접근자가 스캔을 중단시키지 않는다 |
| CI-19 | Phase 3에서 구조 자체가 제거 | **관측 대상이 존재하지 않는다** — 아래 주 참조 |
| CI-20 | Phase 1 수정 | `dist/core/index.d.ts:19,23,32` — `originalValue`. 빌드 산출물로 확인 |
| CI-21 | Phase 4 수정 (`DC-12`) | `narrowing.ts`(8건), `tree-lifetime.ts`, `matrix.ts`(중첩 경로+length) |
| CI-22 | Phase 6.5 수정 (`DC-14`) | `tree-lifetime.ts`(7건), `stress.ts`(1만 인덱스 → 노드 4), 벤치 `ACCUMULATION` 게이트 |
| CI-23 | Phase 7 수정 (`DC-16`) | `hardening.ts` — 연쇄 / 자기 먹이기 / 깊이 누수 (3건) |
| CI-24 | Phase 7 수정 (`DC-17`) | `hardening.ts` — 해제 5시나리오 (5건) |

**단위 테스트가 없는 셋에 대하여.** 종료 조건을 "각 CI에 테스트 1개"로 적었지만, `CI-11`·`CI-19`·`CI-20`은 단위 테스트가 **맞는 도구가 아니다**. 없는 것을 있는 것처럼 세지 않기 위해 여기 남긴다.

- `CI-11`(읽기 O(depth²))과 `CI-19`(불필요한 할당)은 **비용**에 관한 항목이다. 고쳐진 뒤의 동작은 고쳐지기 전과 똑같고, 다른 것은 걸리는 시간뿐이다. 이것을 재는 장치는 벤치 게이트이고, 그것은 `pnpm gate`에 걸려 있다. `CI-19`는 그 위에 구조(`newDepthList`)가 아예 없어져 관측할 대상 자체가 없다
- `CI-20`(공개 시그니처 오타)은 **타입 선언**에만 존재한다. 런타임에서 매개변수 이름은 관측되지 않으므로 어떤 런타임 단언으로도 잡히지 않는다. 확인 지점은 빌드가 내는 `.d.ts`다

나머지 21개는 각각 최소 1개의 테스트가 있고, 그중 8개는 여러 파일이 겹쳐 덮는다.

### 매트릭스 · 스트레스 · 스냅샷 — 완료 (2026-09-18)

`matrix.ts` 98건 + `stress.ts` 11건 신규 (코어 169 → **277**).

**매트릭스에서는 새 결함이 나오지 않았다.** 옵션 16조합(autoSync × editable × cache × trackDeps)을 값 9형태와 함께 돌렸고, 조합마다 다른 동작이 아니라 **모든 조합에서 성립해야 하는 불변식**을 물었다 — 변경 1회당 통보 1회, 같은 값 재대입은 통보 없음, `editable` 계약, `cache` 계약, 프록시 identity, 중첩 경로와 `length`. 얇았던 축(`cache:false`는 저장소에 실사용이 없고 `trackDeps`는 새 opt-in)이 이제 전 조합에서 밟힌다.

**스트레스도 결함 없음.** 깊이 64 / 배열 10,000. 실패한 2건은 **내 테스트가 틀린 것**이었고 제품이 아니었다 — 하나는 조상 교체가 잎을 깨우리라 가정했는데, 통보를 정하는 것은 조상의 참조가 아니라 **구독된 값**이다(`runner`가 경로를 다시 읽는 이유가 이것이다). 다른 하나는 `lens().get`의 시그니처를 잘못 썼다. 둘 다 올바른 계약을 고정하도록 고쳐 남겼다.

**회귀 스냅샷 — 끝내 뒤집히지 않을 약속 3건을 정정했다.** `CI-13`·`CI-14`·`CI-16`이 `SNAPSHOT (wrong): … Phase N이 고칠 것`이라고 적혀 있었다. 셋 다 이후에 **결함이 아니라 설계**로 결론난 항목이다(`DC-03` 배칭 비도입, `DC-02` opt-in 유지, `cache:false`의 정의된 의미). 스냅샷이 계획의 옛 의도를 계속 주장하고 있었고, 그대로 두면 다음 사람이 "아직 안 고쳐진 버그 3개"로 읽는다. `DEFINED (Phase N, DC-xx)`로 다시 적고 각각에 근거 주석을 붙였다.

**기준 테스트**
- 위 전 항목 통과
- 코어 테스트 파일이 CI-01~CI-24를 커버 (추적표 위에 작성). **21/24는 단위 테스트가 있고, 3개는 단위 테스트가 맞는 도구가 아니다** — 표 아래 주 참조

**종료 조건**
- [x] CI-01~CI-24 각각에 검증 지점이 있다 (추적표가 문서에 있다). 21개는 단위 테스트, `CI-11`·`CI-19`는 벤치 게이트, `CI-20`은 빌드가 내는 `.d.ts`
- [x] `pnpm gate`가 존재하고, 실패 주입 3종에서 실제로 막는 것이 확인됐다
- [x] 누수 테스트 통과
- [x] ~~번들 상한(`DC-09` 3,200 B) 유지 — Phase 7은 **테스트만 늘리므로 코어 바이트 무영향**이어야 한다~~ → **전제가 틀렸다.** Phase 7은 테스트만 늘리는 단계일 예정이었으나 결함 두 개를 찾아 고쳤고, 그 수정이 +174 B를 썼다. 상한을 **3,400 B**로 재설정(`DC-09` 3차)하고 현재 3,347 B로 유지한다
- [x] **차분 오라클 무차이** — `CI-24`가 전파 판정을 건드리므로 `dbdc835`를 BASE로 5 시드 × 800 형태 전부 무차이

---

## Phase 8 — 통합 테스트 (커넥터 5종)  ✅ 완료 (2026-09-18) / ⚠️ 육안 항목 2개는 사용자 확인 대기

**진입 조건:** Phase 7 종료 ✅

> **계획 정정 (2026-09-18).** 착수 시점 대조에서 세 항목이 어긋나 있었다.
> - **`batch` 영향 측정 삭제** — `DC-03`이 배칭 비도입으로 종결됐다. 잴 대상이 없다
> - **`DC-03` 기본값 전환 삭제** — 같은 이유. Phase 8이 확정할 기본값은 `DC-02`(`trackDeps`) 하나다
> - **`NFR-3`의 "gzip +15% 이내" 삭제** — `DC-09`가 상대값을 버리고 절대값으로 두 번 재정의했고(4,000 B → 3,200 B → 3,400 B), 지금 기준은 **핀된 Node에서 minified gzip ≤ 3,400 B**다. 커넥터 5종은 별도 상한이 없으므로 **측정해서 기록**하는 것이 이 항목의 실제 내용이다

**체크리스트**
- [x] 클린 빌드 — `pnpm clean:dist && pnpm build`
- [x] `pnpm test:react` / `test:preact` / `test:vue` / `test:svelte` / `test:solid` 전량 통과 — **39 → 68건**
- [x] 각 커넥터에서 렌더 횟수를 baseline과 비교, `trackDeps`의 영향 측정 — 5종 전부 통합 테스트 추가. `trackDeps` ON에서 버려진 분기 리렌더 **1회 → 0회**
- [x] `IC-01` 결과 반영 — 5종 모두 언마운트/파기 후 알림 0회 확인. Phase 5 수정이 커넥터까지 도달한다
- [x] 각 커넥터에 배열 상태 렌더 시나리오 추가 — `CI-10`(`.value` 경유)과 `CI-21`(인덱스 쓰기 → `length`) 양쪽. **커넥터 코드 수정 불필요**
- [x] **`CI-23`·`CI-24`가 커넥터에 닿는지 확인** — React에서 "부모가 같은 패스에서 자식을 떼어내는" 시나리오로 `CI-24`의 실사용 형태를 고정했다
- [x] 코어·커넥터 번들 크기 측정 → 기록. 코어 3,336 B(상한 3,400). 커넥터 gzip: react 296 / preact 304 / svelte 348 / solid 357 / vue 425 B
- [x] `DC-02` 확정 — **3.0.0에서 기본 ON**
- [x] `DC-08` 확정 — **3.0.0**. 커넥터 peer `^2.0.0` → `^3.0.0`도 함께
- [x] `IC-03` 반영 — 4곳 전부. `trackDeps`·해제 방법(양쪽 언어)·`NAVI`/`TYPE`
- [x] CHANGELOG 작성 — `CHANGELOG.md`, CI-01~CI-26 ↔ 사용자 영향 + 마이그레이션
- [x] `MANUAL_TEST_CHECKLIST.md` 수행 — **자동화 가능한 44항목 전부 PASS.** M-03(devtools 육안)과 M-05의 힙 관측은 **미수행, 사용자 확인 필요**

### Vue 커넥터에서 결함 두 개 — `CI-25`, `CI-26` (2026-09-18)

**둘 다 코어가 아니라 커넥터에 있고, 출시된 2.1.0에 있다.** 코어는 정확히 통보했고 커넥터가 그것을 버렸다 — 코어 테스트로는 구조적으로 닿을 수 없는 자리다.

| | `CI-25` | `CI-26` |
|---|---|---|
| 증상 | 컴포넌트 마운트와 **같은 턴**에 도착한 스토어 변경이 조용히 버려진다 | 스토어 값이 `0`·`''`·`false`·`null`이면 리액티브 객체가 **교체**되어 컴포넌트가 영구히 끊긴다 |
| 원인 | 에코 방지 플래그를 **첫 실행**이 올린다. 첫 실행은 되돌려 쓸 게 없는데도 올리고, 해제는 `queueMicrotask`다 | 갱신/생성 분기를 `reactiveValue?.value`의 **참/거짓**으로 판단한다 |
| 실측 | 마운트 직후 쓰기가 DOM에 반영되지 않음 | 0 → 1에서 DOM이 0에 고정, 이후 쓰기는 스토어에도 도달하지 않음 |
| 수정 | 생성은 플래그를 올리지 않는다 | 존재 여부로 판단한다 |

**`CI-26`은 카운터가 0에서 시작하는 가장 흔한 예제에서 바로 터진다.**

**꺼져 있던 테스트 1건이 `CI-25` 때문이었다.** Vue 스위트의 `it.skip`에 "@testing-library/vue가 렌더마다 스토어를 새로 만든다"는 주석이 붙어 있었는데 **오진이었고**, 그 오진이 진짜 결함을 가리고 있었다. `render`를 세 번 부르면 한 턴에 세 번의 마운트가 일어나고, `CI-25`가 그 사이의 쓰기를 전부 삼켰다. 수정 후 통과하여 되살렸고, 주석을 사실로 교체했다.

### 커넥터 통합 — 5종 전부 (2026-09-18)

커넥터 테스트 **39 → 68건**. React/Preact는 렌더 횟수를, Svelte/Solid/Vue는 커넥터가 돌려주는 store·signal·reactive의 갱신 횟수를 센다 — 그 프레임워크에서 렌더를 결정하는 지점이다.

- **narrowing이 프레임워크를 건너서도 유지된다** — 경로가 움직인 컴포넌트만 렌더된다(5종 전부)
- **`CI-10`/`CI-21`이 커넥터 코드에 영향을 주지 않는다** — 배열 교체·인덱스 쓰기·`length` 구독 전부 커넥터 **무수정** 통과
- **React는 코어의 비배칭을 흡수한다** — 한 핸들러 안의 두 쓰기는 코어가 **2회 통보**하지만 React는 **1회 렌더**한다. `DC-03`의 비배칭 결정이 React 사용자에게 렌더 비용을 지우지 않는다는 뜻이다(측정으로 확인)
- **`CI-24`의 실사용 형태** — 부모가 같은 패스에서 자식을 떼어내는 시나리오. 떼어진 자식은 그 패스에서도, 이후에도 렌더되지 않는다
...
**종료 조건**
- NFR-3(코어 ≤ 3,400 B), NFR-4(커넥터 무수정 통과) 충족
...
### 2026-09-18 — Phase 7 진행 중 (게이트 + 전파 경계)
- **done**
  - **`DC-15` 해소.** 이 저장소에 CI가 없다는 것을 착수 시점에 발견 — 계획서가 없는 전제를 깔고 있었다. 게이트의 **정의**(`pnpm gate`)와 **실행 위치**(CI)를 분리하고 후자를 미뤘다. `scripts/gate.mjs`: 빌드 → 타입 → 린트 → 테스트 → 벤치 → 번들, 첫 실패에서 비영 종료
  - **게이트가 실제로 막는지 주입 3종으로 확인** — 타입 오류 / 번들 초과 / 벤치 회귀 전부 중단. 주입은 되돌렸다
  - **`CI-23` 발견·수정** (`DC-16`, 깊이 100 + throw). 자기를 먹이는 구독자가 스택을 터뜨리고 그 `RangeError`를 리포터가 삼키던 자리
  - **`CI-24` 발견·수정** (`DC-17`, 해제된 `run` 건너뛰기). 패스 중 발화한 `AbortSignal`이 피해자 자신의 읽기로 되살아나 **영구 구독**이 되던 자리
  - `src/tests/core/hardening.ts` 12건 (코어 157 → **169**). 실패하는 테스트 3건을 먼저 두고 고쳤다
  - **`DC-09` 3차 상향** 3,200 → 3,400 B (현재 3,347 B, 잔여 53 B)
  - 계획 정정 3건 — 죽은 `batch` 축 삭제(`DC-03` 종결분), `keyIndex` → 경로 트리 기준(`DC-10` 이후 존재하지 않는 구조), "CI 편입" → `pnpm gate`
- **next**
  - **Phase 7 잔여 6항목**: 엣지 케이스 매트릭스, 깊이 64 스트레스, 1만 노드 배열 스트레스, Phase 0 회귀 스냅샷 갱신 + CI-ID 주석, **CI-01~CI-24 추적표**, `docs/core-improvement/` 재현 스크립트 동작 확인
  - Phase 8에서 `DC-08`(semver) 확정 — **`CI-23`이 새 throw를 도입했으므로 등급 산정에 들어간다**
- **blockers**
  - 없음. 미해결 결정은 `DC-08` 하나뿐
- **발견**
  - **번들 수치가 Node 버전에 의존한다.** 같은 산출물이 Node 20.3.0에서 3,347 B, 22.13.0에서 3,320 B. `pnpm exec node`가 볼타 핀(20.3.0)을 우회한다. **27 B는 이 예산에서 변경 하나의 크기**다 — Phase 6.5의 "잔여 8 B"는 어느 Node에서 잰 것인지 알 수 없어 그 자체로 신뢰할 수 없는 숫자였다. 이제 측정이 Node 버전을 함께 출력한다
  - **벤치 회귀 주입이 `childOf`에서는 아무 영향이 없었다.** 6.5-B 이후 구독이 닿지 않는 읽기는 노드를 만들지 않으므로 `childOf`를 호출조차 하지 않는다 — 설계가 의도대로 동작한다는 독립적 확인이다. 읽기 비용은 프록시 `get` 트랩에 있다
  - **Phase 6.5의 경고가 실제로 걸렸다.** "다음에 코어에 뭘 더하려면 `DC-09`의 수치를 다시 판단하라". 정확성·누수 수정이 27 B 때문에 막히는 것은 본말전도라, 수정을 먼저 넣고 상한을 실측으로 재설정했다
  - **계획서의 세 항목이 현행 코드와 어긋나 있었다.** 전부 Phase 2~3 시점의 구조를 가정한 문장이다. 오래된 계획은 그대로 실행하면 안 되고 착수 시점에 대조해야 한다는 것이, 이 프로젝트에서 두 번째로 확인된다
- **commit** `0304dcd` (게이트) + `8dc6e2c` (`CI-23`/`CI-24` 수정). `dbdc835` 위에 쌓임

### 2026-09-18 — Phase 7 완료
- **done**
  - 체크리스트 11항목 전부. 코어 테스트 **157 → 277**, 신규 파일 3종(`hardening.ts` 12 / `matrix.ts` 98 / `stress.ts` 11)
  - **엣지 케이스 매트릭스** — 옵션 16조합 × 값 9형태. 조합별 동작이 아니라 **전 조합 공통 불변식**을 물었다. 새 결함 없음
  - **스트레스** 깊이 64 / 배열 10,000. 새 결함 없음 (실패 2건은 내 테스트가 틀린 것)
  - **회귀 스냅샷 정정** — 뒤집힐 일이 없는 약속 3건(`CI-13`/`CI-14`/`CI-16`)을 `DEFINED`로 다시 적었다
  - **CI-01~CI-24 추적표** 작성. 21개는 단위 테스트, `CI-11`·`CI-19`는 벤치 게이트, `CI-20`은 `.d.ts`
  - **차분 오라클 5 시드 무차이** (`dbdc835` BASE). `CI-24`가 전파 판정을 건드리므로 필수였다
  - `pnpm gate` 전 단계 통과. 번들 3,347 B / 상한 3,400 B
- **next**
  - **Phase 8 — 통합 테스트 (커넥터 5종).** 진입 조건 충족
  - Phase 8에서 `DC-08` 확정 — **`CI-23`의 새 throw가 등급 산정에 들어간다**. `CI-24`는 버그 수정이므로 patch 등급
  - Phase 8 체크리스트의 `DC-02`/`DC-03` 기본값 전환 항목 중 **`DC-03`은 이미 종결**(배칭 비도입)이므로 측정 대상이 아니다
  - CI 편입(`DC-15`에서 미룬 것)은 Phase 8 이후. 워크플로는 `pnpm gate` 한 줄이면 된다
- **blockers**
  - 없음. 미해결 결정은 `DC-08` 하나뿐
- **발견**
  - **테스트를 쓰는 일이 테스트 대상보다 계획서를 더 많이 고쳤다.** Phase 7에서 정정한 계획·문서 항목이 6건(죽은 `batch` 축, `keyIndex`, "CI 편입", 뒤집히지 않을 스냅샷 3건)이다. **오래된 계획은 실행 대상이 아니라 대조 대상**이라는 것이 이 프로젝트에서 세 번째로 확인됐다
  - **매트릭스는 버그를 안 냈지만 그게 결과다.** 2/5 타율로 결함을 물던 앞 항목들과 달리 98건이 전부 초록이었다 — `cache:false`와 `trackDeps`처럼 실사용이 없어 얇던 축이 이제 전 조합에서 밟힌다는 뜻이고, 이후 변경에서 그 축이 깨지면 여기서 걸린다
  - **내 테스트가 두 번 틀렸고 제품은 맞았다.** 조상 교체가 잎을 깨울 것이라 가정한 것이 그중 하나인데, 통보를 정하는 것은 조상의 참조가 아니라 **구독된 값**이다. 실패를 제품 결함으로 보고하기 전에 계약을 먼저 읽어야 한다는 사례로 남긴다
  - **`DC-15`에서 미룬 CI가 Phase 7 내내 아쉽지 않았다.** `pnpm gate` 하나로 충분했고, 미룬 것은 워크플로 파일뿐이지 검증이 아니다
- **commit** `8dc6e2c` 위에 쌓임

### 2026-09-18 — Phase 8 완료
- **done**
  - 커넥터 5종 통합 테스트 **39 → 68건**. 렌더/갱신 횟수, 배열 시나리오(`CI-10`/`CI-21`), 해제 경로를 프레임워크마다 고정
  - **`CI-25`·`CI-26` 발견·수정** — 둘 다 Vue 커넥터, 둘 다 출시된 2.1.0에 있다. 꺼져 있던 테스트 1건이 `CI-25` 때문이었고 되살렸다
  - **`DC-02` 확정** — ~~`trackDeps` 3.0.0 기본 ON~~ → **아래 2026-09-18 후속에서 OFF로 철회.** 커넥터 68건 전부 무수정 통과(NFR-4)는 양쪽 기본값에서 모두 성립
  - **`DC-08` 확정** — `state-ref@3.0.0`. 커넥터 peer `^2.0.0` → `^3.0.0`, 커넥터 minor 올림
  - `CHANGELOG.md` 작성. `IC-03` 반영 — README·stateRefDocs(양쪽 언어)·skills·agent addon
  - **`NAVI`/`TYPE`를 public export로 추가** — `DC-05`가 디버그 핸들을 Symbol로 옮겼는데 **패키지에서 내보내지 않고 있었다.** `Symbol.for`라 우회는 가능했지만 문서화된 표면이 아니었다(+42 B)
  - `MANUAL_TEST_CHECKLIST` — 자동화 가능한 **44항목 전부 PASS**
- **next**
  - **사용자 확인 2건** — M-03(devtools 육안), M-05 #2·#10·#11(브라우저 힙 관측). 둘 다 사람이 봐야 한다
  - 태그·배포는 수행하지 않았다 (`node create-tags.cjs` + publish는 별도)
  - 후속 과제: **차분 오라클에 조건부 읽기 생성 추가** — 지금 구조로는 `trackDeps` 축을 볼 수 없다
  - `DC-15`에서 미룬 CI 편입. 워크플로는 `pnpm gate` 한 줄
- **blockers**
  - 없음. **미해결 `DC`/`IC` 없음** (`DC-01`~`DC-17`, `IC-01`~`IC-03` 전부 해소)
- **발견**
  - **코어를 8개 Phase 동안 파고도, 마지막 Phase가 커넥터에서 더 심한 버그를 찾았다.** `CI-26`은 "카운터가 0에서 시작"이면 바로 터지는데 2.1.0으로 출시돼 있었다. 검증이 코어에 쏠려 있었다는 뜻이고, 커넥터 테스트가 39건뿐이었던 것이 그 증거다
  - **꺼진 테스트의 주석을 믿으면 안 된다.** `it.skip`에 달린 원인 설명이 오진이었고, 그 오진이 진짜 결함을 가리고 있었다. 스킵을 볼 때마다 이유를 **재검증**해야 한다
  - **React가 코어의 비배칭을 흡수한다.** 한 핸들러의 두 쓰기 = 통보 2회 / 렌더 1회. `DC-03`이 배칭을 기각할 때 이 측정이 있었다면 논거가 더 단단했을 것이다
  - **차분 오라클의 "무차이"에는 범위가 있다.** `trackDeps` 기본값을 뒤집었는데도 무차이가 나오는 것은 오라클이 조건부 읽기를 만들지 않기 때문이다. 도구가 침묵한다고 변경이 없는 것이 아니다 — 무엇을 재는 도구인지 매번 확인해야 한다
  - **`DC-05`가 API를 반쪽으로 옮겨놨었다.** 문자열 키를 Symbol로 바꾸면서 그 Symbol을 내보내지 않았다. 결정 하나가 공개 표면을 바꾸면 **표면 전체를** 확인해야 한다
- **commit** `18707b1` (Vue 수정 + 커넥터 통합 테스트), `a1ec127` (`trackDeps` 기본값 + 3.0.0), `ebd139f` (CHANGELOG·문서). `ebdf9fa` 위에 쌓임

### 2026-09-18 — Phase 8 후속 (`DC-02` 철회, 용어 정정)
- **done**
  - **`DC-02`를 기본값 OFF로 철회** (`64e6ca6`). ON으로 정한 근거가 좁은 경우에만 참이었다 — 아래 §발견
  - `trackDeps` 손익분기 실측 → `DESIGN.md` `DC-02`에 표로 기록
  - **용어 정정** (`88689c9`) — "2.x는 opt-in", "off through 2.x" 같은 서술이 **출시된 2.1.0이 이 옵션을 끈 채 나갔다**는 뜻으로 읽혔다. 실제로는 `main`의 `createStore<V>(orignalValue: V)`가 두 번째 인자를 아예 받지 않는다. 코드 주석·README·문서 사이트(양쪽 언어)·skills·addon·CHANGELOG에서 고쳤다
  - `trackdeps-external.tsx` 2건 추가 (`41214ab`) — 커넥터 경유 시 구독이 좁아지지 않는다는 것과 그 이유를 고정
  - CHANGELOG에서 `trackDeps`를 **Changed → Added**로 이동. 아무도 갖고 있지 않던 기본값은 "바뀔" 수 없다
- **next**
  - **사용자 확인 2건** — M-03(devtools 육안), M-05 #2·#10·#11(브라우저 힙 관측)
  - 태그·배포 (`node create-tags.cjs` + publish)
  - 후속 과제: 차분 오라클에 **조건부 읽기 생성** 추가 (지금 구조로는 `trackDeps` 축을 볼 수 없다)
  - `DC-15`에서 미룬 CI 편입 — 워크플로는 `pnpm gate` 한 줄
- **blockers**
  - 없음. 미해결 `DC`/`IC` 없음
- **발견**
  - **기본값을 정할 때 쟀던 것이 "이득이 나는 경우"뿐이었다.** 조건 분기 컴포넌트에서 렌더 1 → 0회는 사실이지만, (a) 커넥터에서는 조건이 **스토어 안**에 있을 때만 좁아지고(재수집은 `run` 안에서만 일어난다), (b) 조건부 읽기가 없는 다수 구독자는 이득 0에 비용 1.40x, (c) 손익분기는 **알림의 약 40% 제거**다. **이득은 좁은 경우에만, 비용은 모두가** — 기본값이 될 조건이 아니었다
  - **깨지는 변경 수는 철회해도 4개 그대로다.** 기본값 ON은 CHANGELOG의 Breaking이 아니라 Changed 절에 있었다. 대화 중 "4 → 3"이라고 말했던 것은 오류이고 정정했다. 철회로 얻는 것은 등급이 아니라 **기본 동작이 2.x와 같아진다**는 점이다
  - **기본값을 되돌리면 그 기능을 쓰던 테스트가 조용히 무의미해진다.** 커넥터 테스트 2건이 기본값에 기대고 있어서 `{ trackDeps: true }`를 명시하지 않으면 통과하지만 아무것도 검증하지 않게 된다
- **commit** `64e6ca6` (최신). `ebdf9fa`(Phase 7) 위의 Phase 8 커밋 8개
