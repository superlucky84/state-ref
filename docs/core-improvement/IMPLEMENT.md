...
## Phase 6.5 — 경로 트리 회수 (CI-22, DC-14)
...
### 6.5-B 구독 시점까지 노드 실체화를 미룸

노드는 **프로퍼티 접근**에서 생긴다(`proxy/index.ts:42`). 반드시 필요한 곳은 `collector` 하나다.

**체크리스트**
- [ ] 프록시가 `pathNode` 대신 `(parentNode, segment)`를 든다
- [ ] `collector` 호출 직전에만 `childOf`로 실체화
- [ ] `pathToString`을 노드 없이 `(parentNode, segment)` 체인에서 계산 (NAVI·inspect·`Symbol.toPrimitive` 에러)
- [ ] 쓰기 전파를 `resolveWrite(pathRoot, segments)` 형태로 — 존재하는 가장 깊은 노드 `D` + 남은 세그먼트 수로 분기 (§3.4-2 표)
- [ ] 남은 세그먼트 **1개**일 때 `D`의 자식에 세그먼트 규칙 적용 (`items.2` 노드가 없어도 `items.length` 구독자 통보 — `CI-21` 경로)

**기준 테스트**
- [ ] **차분 오라클을 확장**: 쓰기 경로의 노드가 없는 경우를 포함 (현재 오라클은 모든 경로가 구독돼 있어 이 분기를 안 탄다)
- [ ] `items.length`만 구독한 채 `items[2]` 쓰기(append) → 통보됨 (노드 미실체화 상태에서)
- [ ] `items[3]`만 구독한 채 `items.length = 2` 쓰기 → 통보됨
- [ ] 쓰기만 한 동적 키 10,000개 → **노드 수 3개** (root + byId + 구독 경로)
- [ ] 구독이 루트까지 체인을 실체화함을 단정 (자손 규칙의 근거)
- [ ] `ref.a === ref.a` 유지 (`CI-15`) — 프록시 memo는 그대로
- [ ] `ref.a.b[NAVI]` / inspect / `Symbol.toPrimitive` 에러 메시지가 노드 없이도 경로를 보고
- [ ] `DC-01` 에러 메시지 무변경

**기존 경계 테스트 전환 (`tree-lifetime.ts`)**
- [ ] `keeps one node per distinct segment ever asked for` → **구독된 세그먼트만** 노드가 된다로 전환
- [ ] `walks dead siblings on a length write` → 죽은 형제가 **애초에 없다**로 전환
- [ ] 불변식 4건(어떤 회수 전략도 깨선 안 되는 것)은 **무변경 통과**해야 한다

---

### 공통 종료 조건
- [ ] 차분 오라클(확장판) 무차이 — 다중 시드
- [ ] **출시 빌드 대비 차분 스윕 무차이** (4 시드 이상)
- [ ] 벤치 게이트 전부 PASS + 신규 게이트: 누적 자식 16,000개에서 부모 쓰기가 깨끗한 스토어와 동급
- [ ] 번들 minified ≤ 3,200 B (`DC-09`)
- [ ] `pnpm test` 전체 통과, 커넥터 5종 무수정
- [ ] `DC-14` 해소, `CI-22` 해소
- [ ] 노드 수·시간을 재는 장치가 벤치에 남을 것 — **`CI-22`를 놓친 이유가 "누적 상태를 재는 게 없었다"이므로 이번엔 게이트로 남긴다**
