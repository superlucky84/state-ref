...
**기준 테스트**
- 위 전 항목 통과
- 코어 테스트 파일이 CI-01~CI-22를 1:1로 커버 (추적표 작성)

**종료 조건**
- CI-01~CI-22 각각에 대응하는 테스트가 최소 1개씩 존재 (추적표가 문서에 있다)
- `pnpm gate`가 존재하고, 실패 주입 3종에서 실제로 막는 것이 확인됐다
- 누수 테스트 통과
- 번들 상한(`DC-09` 3,200 B) 유지 — Phase 7은 **테스트만 늘리므로 코어 바이트 무영향**이어야 한다
...
- **commit** `dbdc835` 기준, 본 커밋이 그 위에 쌓임 (커밋 전)
