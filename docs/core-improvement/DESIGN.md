...
**쓰기 전파의 규칙** (노트 §5-1이 모델로 검증: 3,000 비교 / 불일치 0).

> **구현에서 단순화됐다 (2026-09-17).** 아래 표는 3분기지만, **노드를 프록시를 *지나갈* 때 만들고 *도착할* 때는 만들지 않는다**는 규칙을 세우면 쓰기 노드의 부모는 **항상** 실체화돼 있다. 따라서 "남은 세그먼트 1개" 행만 존재하고 2 이상은 발생하지 않는다. 구현은 `forEachAffected(parent, segment, visit)` 하나로 끝난다.

쓰기 경로를 루트에서 따라가 **존재하는 가장 깊은 노드 `D`** 를 찾고, 남은 세그먼트 수로 갈린다:
...
- [x] **DC-09** 번들 예산 (NFR-3) → **3차 해소 (2026-09-18): 상한 3,200 → 3,400 B. 측정에 Node 버전을 명시한다**

  Phase 6.5가 남긴 경고("다음에 코어에 뭘 더하려면 수치를 다시 판단하라")가 걸린 첫 지점이다. `CI-23`·`CI-24` 수정이 **+174 B**를 쓴다.

  | 항목 | minified gzip (Node 20.3.0) |
  |---|---|
  | Phase 6.5 종료 시점 | 3,173 B |
  | + `CI-24` 해제 수정 | 3,204 B (+31) |
  | + `CI-23` 깊이 가드 | **3,347 B** (+143) |
  | 새 상한 | **3,400 B** (잔여 53 B) |

  - **지불 근거:** 되돌릴 수 없는 누수 하나와, 스스로를 아무것도 아닌 것으로 보고하던 스택 오버플로 하나. 예산은 이런 것을 막으라고 있는 게 아니라 이런 것을 **살 수 있으라고** 있다
  - 메시지를 줄여 8 B를 아끼는 안은 기각했다 — 진단 정보(경로)를 버리는 대가로는 너무 적다
  - **측정이 Node 버전에 의존한다는 것을 이번에 발견했다.** 같은 바이트가 Node 20.3.0에서 3,347 B, Node 22.13.0에서 3,320 B로 나온다(zlib 차이). **27 B는 이 예산에서 실제 변경 하나의 크기다.** `bundle-size.mjs`가 측정한 Node 버전을 함께 출력하도록 고쳤고, 상한은 **volta가 핀한 20.3.0 기준**으로 정의한다
  - 이전 판정(~~3,200 B~~)은 아래에 남긴다
...
- [x] **DC-14** `CI-22` 회수 전략 (`DC-13` 대체) → **해소 (2026-09-17): 6.5-A + 6.5-B. ④ 약참조화 기각**

  **핵심 규칙 — 노드는 프록시를 *지나갈* 때 만들고, *도착할* 때는 만들지 않는다.** 그래서 영속 트리 크기가 "접근된 경로 수"에서 **"구독된 경로 수"** 로 바뀐다.

  | 항목 | 이전 | 이후 |
  |---|---|---|
  | 쓰기만 한 경로 10,000개의 노드 수 | 10,003 | **3** |
  | 영속 `PathNode` 비용 | 495 B/경로 | **0** (미생성) / 구독된 경로는 155 B |
  | 누적 16,000 뒤 부모 쓰기 1회 | 4.70 ms | **0.00 ms** |
  | 누적 64,000 뒤 부모 쓰기 1회 | 22.85 ms | **0.00 ms** |
  | 번들 minified | 3,068 B | **3,192 B** (상한 3,200) |

  - **6.5-A** 노드 필드 지연 할당 — 구독된 노드도 495 → 155 B. 의미 변화 0
  - **6.5-B** 실체화 지연. §3.4-2의 3분기가 위 규칙으로 **1분기로 단순화**됐다 — 쓰기 노드의 부모는 항상 실체화돼 있으므로 "남은 세그먼트 2 이상"이 발생하지 않는다
  - **④ 양쪽 약참조화 기각.** GC 전 구간이 현행보다 6.7배 느리고(55.2 ms vs 8.2 ms) 성능이 GC 타이밍에 의존하며 `CI-15` identity가 관측 가능하게 깨진다. 6.5-A+B가 **결정적으로** 같은 목표를 달성하므로 이유가 없다
  - 검증: 차분 오라클을 **노드 없는 쓰기 경로**까지 확장(5 시드 무차이), 출시 스윕 4 시드 무차이, 뮤테이션 3방향, 벤치 신규 `ACCUMULATION` 게이트, 경계 테스트 6건 신규
  - 내부 seam: `create`가 `pathRoot`를 반환한다. 노드 수는 **구조적으로 외부 관측 불가**이므로(실체화 안 된 경로는 흔적이 없다) 게이트를 만들 다른 방법이 없다. 공개 API로 문서화하지 않는다
  - 근거·측정: §3.4-2, `CI-22-RECLAMATION-NOTES.md`

- [x] **DC-15** Phase 7 게이트의 실행 지점 → **해소 (2026-09-18): 로컬 게이트 스크립트(`pnpm gate`). CI 편입은 후속으로 미룬다**

  **배경.** Phase 7 체크리스트가 타입 게이트와 벤치 게이트를 "CI에 편입"으로 적었으나, **이 저장소에는 CI가 없다** — `.github/workflows`가 존재하지 않고 루트에도 게이트 진입점이 없다. 계획서가 없는 전제를 깔고 있었다.

  - **결정:** 게이트의 **정의**와 **실행 위치**를 분리한다. Phase 7은 정의만 책임진다 — 루트 `pnpm gate` 하나가 타입·린트·테스트·벤치·번들을 순서대로 돌리고 **첫 실패에서 비영으로 종료**한다. 워크플로 파일 작성은 Phase 7의 종료 조건에서 제외한다.
  - **근거:** 게이트 스크립트가 없으면 워크플로는 명령어를 중복 정의하게 되고, 나중에 CI를 붙일 때 그 중복이 갈라진다. 스크립트가 먼저 있으면 워크플로는 `pnpm gate` 한 줄이 된다. 순서가 반대면 두 번 만드는 일이 된다.
  - **기존 자산으로 충분하다:** `bench/read-write.mjs`는 게이트 6/6 실패 시 `exit 1`(`read-write.mjs:177`), `bench/bundle-size.mjs`는 상한 초과 시 `exit 1`(`bundle-size.mjs:83`)로 이미 종료 코드를 낸다. 래핑만 하면 된다.
  - **선행 조건:** 벤치는 `dist`를 직접 import 하므로 게이트는 **빌드를 먼저** 수행해야 한다. 빌드를 건너뛰면 낡은 산출물을 재는 조용한 통과가 된다(Phase 6이 겪은 실패 방식과 같다).
  - **검증:** 게이트가 실제로 막는지 확인한다 — 타입 오류·번들 초과·벤치 회귀를 각각 하나씩 주입해 비영 종료를 확인하고 되돌린다(§6 연결).
  - **미루는 것:** 워크플로 파일, 러너 환경(Node 20.3.0 volta 핀), 캐시 전략. Phase 8 또는 그 이후.

- [x] **DC-16** `CI-23` 구독 콜백의 재귀 쓰기 상한 → **해소 (2026-09-18): (b) 깊이 상한 100 + throw**

  **관측된 현행 동작 (실측).** 연쇄 재진입은 정상이다 — A가 B에 쓰고 B가 C에 쓰면 `a:1 | b:1 | c:1` 순서로 한 번씩 돈다. 깨지는 것은 **자기를 먹이는 루프**다: 구독자가 자기가 읽는 경로에 쓰면 `RangeError: Maximum call stack size exceeded`가 나고, `reportPassErrors`가 그것을 삼키고(`runner.ts:126`), 삼키면서 부르는 `console.error`가 스택이 꽉 찬 상태라 **다시 오버플로한다**. 사용자가 받는 것은 중간에 멈춘 스토어(`n = 956`)와 콘솔의 `AggregateError` 잔해뿐이다.

  | 후보 | 비용 | 문제 |
  |---|---|---|
  | (a) 문서화만 — 사용자 코드 버그로 규정 | 0 B | 침묵 실패가 그대로 남는다. 스토어가 중간 값에 멈춘 것을 알 방법이 없다 |
  | (b) 깊이 카운터 + 상한 초과 시 throw | ~40~80 B | `DC-09` 잔여 27 B를 넘긴다. 상한값이 자의적이다 |
  | (c) 깊이 카운터 + 상한 초과 시 전파 중단 + `console.error` 1회 | ~40~80 B | 위와 같고, 중단 지점이 여전히 임의적 |

  - **연쇄 재진입 자체는 막으면 안 된다** — `INV-4`(동기 1회 통보) 아래에서 정상 용례이고 테스트가 이미 의존한다
  - `DC-08` 연관: (b)는 새 throw를 도입하므로 등급 산정에 들어간다
  - 검증: `src/tests/core/hardening.ts` 3건 — 연쇄가 계속 동작함 / 자기 먹이기가 상한에서 멈춤 / **깊이 카운터가 패스 사이에 누수되지 않음**(300회 얕은 쓰기가 상한 100을 건드리지 않는다)

  **채택 (b), 상한 100.** 실측 근거: 자기 먹이기 루프는 Node 20에서 약 **1,295 프레임**에서 `RangeError`가 났다. 100은 실제 연쇄보다 한참 위이고 스택이 바닥나는 지점보다 한참 아래여서, **에러를 보고할 스택이 남아 있는 동안** 도착한다.

  - 구현: `runner.ts`에 모듈 수준 `passDepth` + `try/finally`. `finally`가 필수다 — 구독자가 던져도 깊이가 돌아와야 한다
  - **에러가 대입문까지 올라가지 않는 것은 `CI-05`의 기존 설계 그대로다.** 구독자 안에서 던져진 것은 `reportPassErrors`가 모아 `console.error`로 낸다. 달라진 것은 내용이다 — 자기 자신 때문에 다시 오버플로하는 `RangeError` 대신, 경로와 원인을 담은 우리 메시지가 나온다
  - 비용: **+143 B** (3,204 → 3,347). `DC-09` 상향으로 지불

- [x] **DC-17** `CI-24` 전파 중 `abort()`의 되살아남 → **해소 (2026-09-18): 해제된 `run`을 그 패스에서 건너뛴다**

  **원인.** `runner`가 `runableRenewList`를 순회하는 도중 앞선 구독자가 다른 구독의 `abort()`를 부르면 `removeRun`이 즉시 지운다. 그런데 그 구독은 **이미 이번 패스의 실행 목록에 들어 있어서** 뒤이어 실행되고, 콜백이 경로를 읽는 순간 `collector`가 **다시 등록한다**. 되살아난 구독은 영구적이다 — signal은 이미 발화했고 리스너는 소비됐다.

  | 시나리오 | 현행 |
  |---|---|
  | 패스 밖 `abort()` | 정상 해제 |
  | 자기 자신 `abort()` (읽은 뒤) | 정상 해제 |
  | `false` 반환 해제 | 정상 해제 |
  | **패스 중 타인이 `abort()`, 피해자가 뒤에서 실행되며 읽음** | **영구 구독** |
  | 패스 중 `abort()`, 피해자가 아무것도 읽지 않음 | 정상 해제 |

  - **수정:** `runner`의 실행 루프에서 `storeRenderList`에 더 이상 없는 `run`을 건너뛴다(`if (!run || !storeRenderList.has(run))`). 해제된 구독은 그 패스에서 실행되지 않고, 실행되지 않으면 **재등록될 경로 자체가 없다**
  - 검증: 위 표 5행 전부가 `src/tests/core/hardening.ts`에 있다. 수정 전 **2건 실패 → 수정 후 5건 통과**를 확인했다
  - 비용: **+31 B** (3,173 → 3,204). 누수 수정 중 가장 싼 축에 든다
  - **`CI-06`/`CI-07`과 같은 계열이다** — 해제했는데 구독이 남는다. 다만 원인이 반대쪽이다: 그 둘은 signal이 코어에 **도달하지 못했고**, 이것은 도달해서 **처리된 뒤 되살아난다**
