...
- [x] **DC-02** CI-14 dep 재수집 기본값 → **3차 해소 (2026-09-18): 기본값 OFF, opt-in 유지.** ~~3.0.0에서 기본값 ON~~ 철회

  **ON으로 정했다가 되돌렸다.** 근거였던 "조건 분기 컴포넌트에서 렌더가 준다"가 **좁은 경우에만 참**이라는 것이 이후 측정에서 드러났다.

  | 판단 시점 | 알고 있던 것 | 결정 |
  |---|---|---|
  | Phase 8 초 | React 조건 분기에서 리렌더 1 → 0회 | ON |
  | 이후 측정 | 커넥터는 **조건이 스토어 안일 때만** 좁아진다 / 조건부 읽기가 없으면 이득 0에 비용 1.40x / 손익분기 ≈ 알림 40% 제거 | **OFF** |

  - **이득은 좁은 경우에만 나고 비용은 모두가 낸다.** 기본값이 될 조건이 아니다
  - 이득을 보는 쪽(분기 조건이 스토어 안에 있는 구독자)은 `{ trackDeps: true }` 한 줄로 켠다
  - **깨지는 변경 수는 그대로 4개다.** 기본값 ON은 `CHANGELOG`의 Breaking 절이 아니라 Changed 절에 있었으므로, 되돌려도 4건(Symbol 핸들·배열 타입·구독자 예외·manual-sync 우회)은 그대로 남는다. `DC-08`(3.0.0)도 그대로다. 되돌린 이득은 등급이 아니라 **기본 동작이 2.x와 같아진다**는 것이다 — 업그레이드하는 사람이 겪을 변화가 하나 줄어든다
  - 코어에서 관측된 누락 가능 경로(바깥 변수로 분기하는 순수 `watch`)도 **기본값에서는 발생하지 않는다**
  ...
  **재수집이 알림을 놓칠 수 있는가 — 실측 (2026-09-18).** 구독을 *좁히는* 변경이므로 방향은 "누락이 는다" 쪽이다. 어디까지인지 재봤다.

  | 조건이 있는 곳 | 결과 |
  |---|---|
  | **스토어 안** (`s.flag.value ? s.a : s.b`) | 안전. 조건이 바뀌면 그 쓰기가 콜백을 깨우고 그 실행에서 목록이 갱신된다. 불필요한 알림만 사라진다 |
  | **스토어 밖 + 커넥터 경유** (컴포넌트 state·props) | **안전.** 재수집은 **스토어 변경이 `run`을 호출할 때만** 일어난다. 컴포넌트 state로 인한 리렌더는 `run`을 거치지 않으므로 그 실행의 읽기는 목록에 **더해질 뿐 빠지지 않는다** |
  | **스토어 밖 + 순수 `watch` 콜백** | **놓칠 수 있다.** 바깥 변수가 분기를 정하고, 그 변수가 바뀌어도 콜백을 다시 돌릴 계기가 없으면 직전 실행에서 빠진 경로는 다시 등록되지 않는다 |

  - 검증: `connect-react/src/tests/react/trackdeps-external.tsx` 2건 — 컴포넌트 state로 분기를 왕복시켜도 쓰기가 도달하고, 목록이 좁아지지 않는다는 것을 각각 고정했다
  - **부수 효과 하나.** 같은 이유로 커넥터에서는 `trackDeps`의 **이득도 조건이 스토어 안에 있을 때만** 나타난다. 컴포넌트 state로 분기하는 컴포넌트는 3.0.0에서도 2.x와 같은 횟수로 렌더된다

  **손익분기 실측 (2026-09-18).** 쓰기 20,000회 / 구독자가 읽는 경로 8개, warm-up 후 5회 중앙값.

  | 시나리오 | OFF | ON | |
  |---|---|---|---|
  | 조건부 읽기 없음 (**가장 흔한 모양**) | 18.8 ms | 26.4 ms | **1.40x 손해** |
  | 버려진 분기에만 쓰기 (알림 100% 제거) | 6.8 ms | 3.5 ms | 0.52x 이득 |
  | 알림 10% 생존 | 6.9 ms | 4.2 ms | 0.61x |
  | 알림 30% 생존 | 6.7 ms | 5.0 ms | 0.75x |
  | 알림 50% 생존 | 6.7 ms | 6.3 ms | 0.94x |
  | 알림 70% 생존 | 6.8 ms | 7.4 ms | 1.09x 손해 |
  | 알림 100% 생존 | 6.9 ms | 9.4 ms | 1.37x 손해 |

  - **손익분기는 "알림의 약 40%가 사라질 때"다.** 그만큼 줄지 않으면 재수집 비용이 이긴다
  - **조건부 읽기가 없는 구독자는 이득이 0이고 비용만 1.40x 낸다.** 그리고 그것이 다수다
  - 다만 절대값은 알림 1회당 **1µs 안팎**이다. 20,000회에 7 ms 차이라 대부분의 앱에서는 성능이 아니라 **의미**로 판단할 사안이다
  - **`DC-02`의 기본값 ON은 이 측정 이후 근거가 약해졌다.** Phase 8의 판단 근거는 "조건 분기 컴포넌트에서 렌더가 준다"였는데, 커넥터에서는 조건이 스토어 안에 있을 때만 그렇고(위), 그 외에는 이득 없이 비용만 낸다. 재검토 시 이 표가 근거다
  - **차분 오라클은 이 변경을 볼 수 없다.** 오라클이 만드는 구독자는 매번 **고정된** 경로 집합을 읽어서(`diff-vs-released.mjs:129`), 재수집해도 같은 집합이 나온다. 여기서의 "무차이"는 변경이 없다는 뜻이 아니라 **오라클이 이 축을 생성하지 않는다**는 뜻이다. 검증은 코어 단위 테스트 2건(기본 ON / opt-out)과 위 React 측정이 맡는다 — 조건부 읽기를 만드는 오라클 확장은 후속 과제로 남긴다
...
> **후속 (2026-09-18, 3.0.1 / `CI-27`).** `DC-11`이 정의한 "소스 변경당 1회"는 맞았지만, `CI-07`의 `equals` 비교를 **구독마다가 아니라 computed마다** 걸어 두어 **첫 구독자 외에는 아무도 못 받았다.** `result`와 proxy가 `createComputed`의 클로저에 있었기 때문이다. 구독마다 하나씩 갖도록 옮겨 해소했다.
>
> 남는 교훈: **헬퍼의 "여러 번 구독" 축이 검증에 없었다.** `createComputed` 테스트가 전부 구독 1개였고, 차분 오라클은 헬퍼를 거치지 않는다. 헬퍼에 상태를 두면 그 상태가 **무엇 단위인지**(computed 단위인지 구독 단위인지)를 테스트로 고정할 것.
...
- [x] **DC-08** semver 등급 → **해소 (2026-09-18, Phase 8): `state-ref@3.0.0`**
  - `DC-03` 해소로 CI-13은 등급 산정에서 빠졌다. 남은 판단 근거는 **사용자 코드를 깨뜨릴 수 있는 변경 4건**이다

  | 깨지는 변경 | 근거 |
  |---|---|
  | 디버그 핸들 `_value`/`_navi`/`_type` → Symbol | `DC-05`. 문자열 키를 읽던 코드가 깨진다 |
  | `StateRefStore<T[]>` 타입 축소 | `DC-04`. TS 사용자의 빌드가 깨진다(문서가 가르친 `.value` 패턴은 통과) |
  | 구독자 예외가 대입문으로 전파되지 않음 | `CI-05`. 대입문에서 잡던 코드가 못 잡는다 |
  | manual-sync + `{cache:false}` 쓰기 우회 차단 | `CI-01`. 우회에 의존한 코드가 throw를 받는다 |

  - 넷 다 **버그 수정**이지만 기존 동작에 의존한 코드는 깨진다. semver 정의상 major다
  - `DC-02`(기본값 ON)도 이 major에 함께 실린다 — 렌더 횟수 감소 역시 동작 변경이다
  - **커넥터의 peer 범위를 함께 넓혔다.** 5종 모두 `state-ref: ^2.0.0`을 못박고 있어 코어 3.0.0과 설치가 충돌한다 → `^3.0.0`. 커넥터 버전은 minor 올림(react 18.3.0, preact 10.3.0, vue 3.3.0, svelte 4.3.0, solid 1.3.0)
  - **⚠️ 남는 긴장.** 커넥터의 major 자리는 **프레임워크 버전**이 쓰고 있어서(react 18.x, vue 3.x) "코어 major 요구"를 major로 표현할 방법이 없다. 커넥터 minor 업그레이드가 코어 major를 강제하는 셈이라 semver를 엄격히 읽으면 어긋난다. 이 스킴을 바꾸는 것은 릴리스 정책 사안이므로 **기록만 남긴다**
  - 태그·배포는 수행하지 않았다(`create-tags.cjs`는 별도 실행)
