...
| 10 | React | 긴 리스트(1,000행)를 반복 교체·필터 20회 | 화면 정합 유지. **힙이 계속 증가하는 것은 알려진 특성**(`CI-22`/`DC-13`) — 실패로 기록하지 말고 증가량만 남긴다 | |

**PASS 기준:** 1~8 필수. 9는 해당 옵션을 릴리스에 포함할 때만. 10은 관측 기록용(판정 없음).
