...
기준일: 2026-09-21. 저장소 `/Users/superlucky84/project/state-ref`, 브랜치 `feat/server-sync-draft`. Phase 5.6의 시작 기준 커밋은 Phase 5.5의 `41c9f42` (`feat(connectors): add readonly query views`)이고, 구현·검증·문서는 현재 작업 트리에 있다. 작업 재개 시 `git log -1 --oneline`과 `git status --short --branch`로 최신 커밋과 작업 트리를 확인한다.
...
주요 코드 위치: [core batch](../../packages/state-ref/src/batch/index.ts), [draft](../../packages/state-ref/src/draft/index.ts), [sync query/client](../../packages/sync/src/index.ts), [자동 재조회](../../packages/sync/src/automatic-refetch.ts), [view](../../packages/sync/src/view.ts), [live view](../../packages/sync/src/live-view.ts), [hydration 형식](../../packages/sync/src/hydration.ts), [resource 기록](../../packages/sync/src/resource.ts), [mutation](../../packages/sync/src/mutation.ts). 소비자 예제는 [sync README](../../packages/sync/README.md)를 따른다.
...
- 고정 Node 20.3.0의 기본 core 기준은 **3,455/3,500 B PASS**다. 이번 gate 경로 Node 24.11.1 측정은 **3,433/3,500 B PASS**다. Phase 5.6 별도 sync ESM은 고정 Node 20.3.0에서 **46,947 B raw / 12,114 B gzip**이며 기본 core 빌드에 포함되지 않는다.
- 구현을 바꾸면 해당 패키지 테스트·타입을 먼저 실행하고 전체 `pnpm gate`로 종료한다. 문서 변경은 `git diff --check`와 링크 경로를 확인한다.

Phase 5.5 구현·테스트·문서 커밋은 `41c9f42`다. Phase 5.6 구현·테스트·문서는 현재 작업 트리에 있다.

## 다음 단계와 완료 기준

1. **Phase 5 계속:** [Phase 5.6](./PHASE5_6.md)에서 client별 focus/reconnect/polling의 정책·공유·정리 계약을 자동 검증했다. 다음은 pagination/infinite의 page identity, 취소, 병렬 page, cache/view, SSR 계약을 고정하는 것이다. 이름만 같은 API를 동등성으로 세지 않는다.
2. 이어서 영속화/오프라인/재개, network mode, browser adapter 제품화와 관측·플러그인 경계를 실제 런타임 반례·타입/번들 증거와 함께 구현한다. 로컬 편집·미확정 작업의 복원 형식은 clean SSR snapshot과 별개로 설계한다. Phase 5 전체 종료 조건은 [IMPLEMENT](./IMPLEMENT.md)의 F2별 증거이며, 일부 기능을 구현해도 전체 동등성 완료로 표시하지 않는다.
