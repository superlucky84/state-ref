...
- **NFR2-01** 기본 코어 진입점은 draft 구현이나 sync 패키지, 네트워크·캐시·전송 구현을 import하지 않는다. core 단독 소비자의 빌드에는 이 구현과 런타임 의존성이 들어가지 않고, draft 진입점도 sync 엔진을 로드하지 않는다. ref 연결이 필요하면 서버 의미가 없는 opt-in 코어 인터페이스로 한정하고 기본 코어의 기존 번들·성능 예산 안에서 검증한다. draft 진입점의 크기는 별도로 측정한다.
