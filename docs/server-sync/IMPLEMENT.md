...
- [x] IC2-06과 F2-01~09의 누락·차이·지원 단계 목록을 갱신한다. 교차 확인 표는 [Phase 8.6](./PHASE8_6.md)에 있고 근거 파일의 실재는 gate의 `support-table` 단계가 확인한다. 지원 5행·부분 지원 4행이며 전체 동등성은 선언하지 않는다.
...
### 2026-09-26 — Phase 8.8 5단계, 주입으로 확인한 실패의 세 모양

- done: **[Phase 8.8](./PHASE8_8.md) 5단계.** `connect-svelte`에 결함 2종(알림 한 번씩 건너뛰기, 원시값 잎만 push)을 주입하자 둘 다 **svelte만** 실패하고 나머지 넷은 전 단계를 통과했다. 실패 메시지는 단계 번호와 그 단계를 둔 이유를 함께 말한다.
- **심어 넣은 커넥터 결함은 두 번 모두 커넥터 스위트가 먼저 잡았다.** 장치의 실패가 아니라 스위트에 대한 좋은 소식이고, [CI-29](../core-improvement/REQUIREMENTS.md)가 왜 스위트를 통과했는지도 말해 준다 — CI-29는 아무 데나 있는 결함이 아니라 **Vue 스위트의 특정 빈칸**(한 턴에 잎 여러 개를 쓰는 축)에 있었다. 이 장치는 유일한 그물이 아니라 **두 번째 그물**이며, 다른 그물이 못 하는 두 가지를 한다: 한 화면으로 범위를 좁히는 것, 그리고 **어떤 기대값도 이름 붙이지 않은 차이**를 잡는 것.
- **두 단언이 독립임을 확인했다.** solid에서만 `memo`(어떤 기대값도 보지 않는 필드)에 접미사를 붙이자 기대값은 다섯 종 모두 통과하고 **일치 단언만** `solid differs from react at step 3`으로 실패했다. 뒤집으면 fixture를 되돌린 앞선 주입에서는 다섯이 **함께** 기대값을 실패하고 일치는 유지됐다. 그래서 실패의 모양이 원인의 위치를 말한다 — 한 종만 기대값 실패는 그 커넥터, 다섯이 함께 실패는 fixture·라이브러리(또는 기대값), 기대값은 통과하고 일치만 실패는 아무도 보지 않던 자리의 화면 차이.
- 주입 3종 모두 원본과 `diff`로 대조해 복원했다. 수치 불변 — `pnpm test:e2e` **15/15**, `examples/shared` **55**, `pnpm gate` 19단계 PASS.
- next: 6단계(항목 이식). React로 이미 통과한 7개를 시나리오로 옮겨 다섯 종으로 넓히고, 미수행 12개를 체크리스트 순서로 채운다. **기대값은 반드시 probe로 측정한 문자열에서 쓴다.**
- blockers: 없다.
- 시작 기준 commit: `635b061`.
...
- **라이브러리 결함 CI-29를 찾아 고쳤다 — 이 장치가 찾은 첫 라이브러리 결함이고, Phase 8.7 이후 처음으로 `packages/` 아래를 건드린 변경이다.** `작업과 정책` 카드가 Vue에서만 항상 **이전 조작**을 보여줬다. 데모의 `bump()`가 한 턴에 잎 셋을 쓰는데 `connectVue`의 에코 방지 가드가 **인바운드 갱신에서** 올라가고 microtask에서야 풀려 두 번째 이후 쓰기가 막혔다. `CI-25`가 같은 실수를 첫 실행에서 고쳤고 갱신 경로는 남아 있었다. **출시된 3.3.0에 있다.** 커넥터 스위트 35개·코어 338개·`examples/shared` 52개·gate 19단계가 전부 통과하는 동안 화면은 뒤처져 있었다 — 커넥터 테스트가 한 턴에 잎 하나만 쓰고 있었기 때문이다. 브라우저에서 찾고, 커넥터 스위트에 반례를 먼저 써서 격리한 뒤 고쳤다(Vue 35 → **36개**). 전체 기록은 [CI-29](../core-improvement/REQUIREMENTS.md).
- **첫 수정 시도가 틀렸다.** 가드를 아예 없애자 Vue 화면이 `pending / fetching`에 멈췄다 — `reactive`가 템플릿에 값의 프록시를 주므로 write-back의 동등성 검사가 자기 값을 못 알아보고 스토어에 clone을 덮어썼다. 가드는 필요하고 틀린 것은 인바운드가 그것을 올린다는 점이었다. **증상이 사라진 것과 원인을 맞힌 것은 다르다.**
- **데모 결함 [B8-7-15](./MANUAL_TEST_CHECKLIST.md#b8-7-15)도 같은 자리에서 나왔다.** `사무실`(객체) 행이 React·Preact·Solid에서는 `JSON.stringify`, Vue에서는 들여쓴 JSON, **Svelte에서는 `[object Object]`**로 그려졌다. 로드 전에는 값 행이 없어 초기 대조에 안 걸렸다. 문자열화를 공유 계약의 `show()`로 옮겼다.
- **DC8-04를 개정한 DC8-08과 짝이 되는 실무 결정 DC8-8-10을 세웠다.** 다섯 프레임워크의 배치가 다르므로 판독은 **정착된 화면**에서 해야 한다(두 번 연속 판독이 같을 때). 이 판정을 빼먹어 Vue를 한 조작 뒤처진 것으로 잘못 읽은 적이 있고, 판독 사이 간격이 없으면 같은 프레임을 두 번 읽어 정착처럼 보인다.
- 수치: Vue 커넥터 35 → **36**, 그 외 불변(core **338**, sync **183**, React **36**, Preact **27**, Svelte **26**, Solid **25**, `examples/shared` **52**). 조작 **45개**. `pnpm gate` 19단계 PASS, `pnpm check:examples` PASS, `pnpm test:e2e` 12/12.
- next: 3단계의 남은 절반 — `examples/shared/src/scenarios.ts`에 시나리오(누를 조작 + 기대 `ScreenReading`)와 vitest용 모델 판독기. 같은 기대값을 모델 판독과 DOM 판독이 함께 통과해야 하므로, 갈리면 결함이 모델인지 렌더인지 그 자리에서 좁혀진다(DC8-8-02). 첫 시나리오는 M2-11 항목 1·2·6.
- blockers: 없다. M2-11의 결과란은 여전히 미수행이다.
- 시작 기준 commit: `03134c4`.

### 2026-09-26 — Phase 8.7 M2-10·M2-11 계측과 Phase 8.8 계획

- done: **M2-10 통과(R2-12 기준, 네 항목).** 저장이 성공하고 기준 복구만 실패한 화면이 두 사실을 따로 말했다 — 서버 `부산 / 2`, 패널 `error`·`unconfirmed=true`·`dirty=true`. 복구는 재조회로 되고 WRITE는 1회 그대로였다. settled `unknown`과 확정 거절은 **서버 상태가 둘 다 `서울 / 1`로 같은데** `unconfirmed`와 제출 입력 되돌림으로 갈렸다 — 화면을 가르는 것은 서버가 한 일이 아니라 클라이언트가 들은 말이다. [체크리스트 M2-10](./MANUAL_TEST_CHECKLIST.md#m2-10). 수행 중 찾은 계측 결함 [B8-7-13](./MANUAL_TEST_CHECKLIST.md#b8-7-13)은 같은 세션에 해소했다([DC8-5-47](./PHASE8_5.md)) — 요청 카드가 **재시도로 생긴 요청을 그리지 않아** 수행자를 체인 중간에 멈추게 했다. "조작마다 tick을 올린다"는 것은 "사람이 누른 것만 화면에 나온다"는 뜻이고, 재시도·자동 조회가 있는 데모에서는 틀린 화면이다.
- **M2-11은 범위를 셋으로 좁혔다([단계 14](./PHASE8_5.md)).** 클릭 절차를 쓰기 전에 코드로 먼저 걸어 보니 `beginLink()`가 epoch를 올리면서 **진행 중 READ를 abort까지 한다**(`packages/sync/src/index.ts:651`). 예의 바른 transport에서는 "늦게 완료"될 기회가 없으므로 첫 항목은 그 **중단**으로 확인되고, 둘째 항목(signal 무시)이 `await queryFn` **뒤의** epoch 검사(`index.ts:772`)를 시험하는 유일한 길이다. 셋째·넷째·다섯째는 `liveView`·query key 전환·복수 mutation이 데모에 없어 **미수행으로 남긴다**(데모 한계로 닫지 않는다).
- **B8-7-14를 해소했다([DC8-5-50](./PHASE8_5.md)):** 요청 기록의 key가 전부 `'profile'`로 박혀 있어 다른 key인 readonly 조회의 READ도 `profile`로 찍혔고, **다섯 데모는 그 열을 아예 그리지 않았다.** sync가 `queryFn`에 주는 컨텍스트는 `{ signal }`뿐이므로(`index.ts:178`) 모델이 조회마다 key를 묶어 넘긴다. 계측 결함이며 라이브러리 문제가 아니다.
- 계측 셋을 더했다 — READ가 **접수 시점 값을 스냅샷으로** 답하고(DC8-5-48; 그래야 늦은 결과가 현재 기준과 달라 판정이 가능하다), `nextReadIgnoresSignal`이 abort를 듣지 않는 READ를 예약하며(DC8-5-49), 조회의 거절을 장벽이 아닐 때도 말한다(DC8-5-51). 조작 44 → **45개**, `examples/shared` 46 → **52개**. 결함 4종 주입이 4종 모두 잡혔고, 그중 **한 주입은 처음에 헛돌았다** — 템플릿 앞부분만 바꿔 테스트가 읽는 문구가 그대로 남았다. 주입이 테스트가 읽는 값을 바꿨는지 먼저 확인해야 한다. `pnpm gate` 19단계 PASS, `pnpm check:examples` PASS(다섯 데모 45개). **`packages/` 소스 변경 0.**
- **수행 속도를 실측해 방향을 바꿨다.** 체크리스트에 기록된 결과는 `React 데모` 8회뿐이고 다른 네 프레임워크는 **0**이다. 8.7의 전제가 "모델은 공유하니 화면이 갈리면 커넥터 탓"이라면 실제 행렬은 **20항목 × 5데모 = 100칸**이고 세 세션이 채운 것은 7칸이다. 손으로는 끝나지 않는다. [Phase 8.8](./PHASE8_8.md)에 **실제 브라우저 수행 장치**를 계획했다 — Playwright의 Chromium에서 실제 DOM 이벤트로 다섯 데모를 같은 시나리오에 걸고, 단언을 **렌더된 DOM에서** 읽어 다섯 화면을 서로 대조한다. 시나리오는 하나이고 판독기가 둘이라(vitest는 모델 투영, e2e는 DOM) 결함이 모델인지 렌더인지 그 자리에서 좁혀진다.
- **DC8-04를 개정했다([DC8-08](./PHASE8.md)).** 자동이 브라우저를 대체할 수 없다는 금지의 근거는 그 자동이 전부 **브라우저가 아닌 실행기**(타입·빌드·모듈 그래프·jsdom·Node 렌더)였기 때문이다. 실제 브라우저 실행은 같은 종류의 증거이므로 증거로 센다 — 단, DOM에서 읽은 단언만, 칸에 출처(SHA·브라우저 버전·시나리오)를 적고, 레이아웃·가독성·DevTools 전용 신호는 사람이 본다. jsdom에 대한 금지는 그대로다.
- next: **Phase 8.8 1단계(장치 뼈대)**. `@playwright/test`(chromium) 설치, 다섯 `webServer`, `pnpm test:e2e`. 3단계의 첫 시나리오는 이미 계측이 들어간 M2-11 항목 1·2·6이다. 그다음 React로 이미 통과한 7개를 다섯 종으로 넓혀 회귀 감시를 만들고, 미수행 13개를 항목 순서로 채운다.
- blockers: 없다. M2-11 결과란은 미수행이고, 8.8의 장치가 그 첫 사례가 된다.
- 시작 기준 commit: `4b8c760`.

### 2026-09-25 — Phase 8.7 M2-09와 거절 경로 fixture

- done: **M2-09 통과(R2-11 기준, 항목 1·2).** 확정 거절에서 `keep`과 `remove` 두 정책을 같은 흐름으로 비교했다 — 수용 방식은 둘 다 `submitted`이고 `onReject`만 다르다. `keep`은 제출한 `도시 → 부산`을 유지하고, `remove`는 그것만 되돌리며 제출하지 않은 `memo` 편집은 남긴다. 저장 시작 **뒤에** 넣은 같은 경로 입력(`도시 → 광주`)이 `서울 → 광주`로 새 기준 위에 다시 얹히고, WRITE와 무관한 서버 갱신(`서버 메모 3`)이 복구를 통과했으며, 먼저 성공한 저장의 기준(`부산`)이 나중 거절에 지워지지 않았다(`서울`로 돌아가지 않았다). 거절된 작업의 자동 재전송은 없었다.
- **B8-7-05를 해소했다([DC8-5-38](./PHASE8_5.md)):** `다음 WRITE 확정 거절 예약`이 **실제로는 `unknown`을 만들고 있었다.** mock이 평범한 `new Error(...)`로 reject했고, sync는 `MutationRejectedError`인 실패만 `rejected`로 분류한다(`packages/sync/src/mutation.ts:321`) — 전송 오류는 서버가 저장했는지 말해 줄 수 없기 때문이다. **라이브러리는 명세대로 동작했고 틀린 것은 fixture다.** 그 탓에 R2-11의 두 정책과 R2-12의 "확정 거절과 unknown 구별"이 화면에서 도달 불가였다. `pnpm gate` 19단계가 전부 통과하는 상태에서 코드 대조로 찾았고, 해소 뒤 화면에서 `mutation phase = rejected`를 확인했다.
- **B8-7-09를 해소했다([DC8-5-41](./PHASE8_5.md)):** `고정` → `제출 뒤 추가 입력` → `저장` 순서로 누르면 `mutation.start`가 `Submission is stale.`로 던지고 **예외가 `run()` 밖으로 나가 클릭 핸들러가 터졌다.** 지역 편집이 resource revision을 올리고(`resource.ts:137`) `mutation.start`가 어긋난 제출을 거절하는 것은(`index.ts:1431`) 올바른 계약이며, 틀린 것은 그것을 받지 않은 데모다. 저장 4종을 `startSave` 헬퍼로 모아 방어가 한 곳에만 있게 했다.
- **계측 부재 셋을 구분해 처리했다:** B8-7-06(`remove` 정책에 도달할 길 없음)과 B8-7-07(서버를 WRITE 없이 바꾸는 조작 없음)은 조작 2개를 더해 해소했고, B8-7-08(부모 생성 fixture 없음)은 **데모 한계로 닫았다**([DC8-5-42](./PHASE8_5.md)) — 그 동작은 `packages/sync/src/tests/mutation.test.ts:285`가 T2-11로 고정한다. 조작 수가 이미 43개라 여기서 멈췄다.
- **거절 뒤 `invalidated`가 `true`로 남는 것을 확인했다.** `beginLink()`가 연결 WRITE 시작 시 epoch을 올리며 무효로 표시하고 성공 경로에서만 내린다. 거절은 기준을 확정하지 못했으므로 계약대로다.
- 조작은 41개에서 **43개**, `examples/shared` 테스트는 32개에서 **38개**가 되었다. 결함 3종을 주입해 3종 모두 새 테스트가 잡는 것을 확인했다. 다른 수치는 불변이고 `pnpm gate` 19단계·`pnpm check:examples` 모두 PASS다. **`packages/` 소스 변경 0.**
- next: M2-10(저장 결과 구분 — `sync-error`/`rejected`/`unknown`)으로 이어간다. `다음 WRITE 결과 불명 예약`과 `다음 WRITE 성공 + 복구 READ 실패 예약`이 거기서 처음 쓰인다. M2-04의 남은 둘은 Vue SSR 데모(5192)와 렌더 계측 차례다.
- blockers: 없다. B8-7-01~07·09 모두 해소했고 B8-7-08은 데모 한계로 닫았다.
- 시작 기준 commit: `0ace662`.

### 2026-09-25 — Phase 8.7 수용 방식 세 가지와 M2-07·M2-08

- done: **M2-08 통과, M2-07 통과(R2-09 기준), M2-06 부분 수행.** 제출한 변경만 확정되고 미제출·후속 입력이 보존되는 것을 화면에서 확인했다 — WRITE가 떠 있는 동안 넣은 zip 입력이 `3 zip "01"→"96"`으로 더해지고 성공 뒤에도 남았다. 고정이 zip 편집 **이전**이라 DTO는 옛 값을 실어 갔고, 그래서 제출값과 제출 후 입력이 섞이지 않았다.
- **세 수용 방식의 READ 비용이 갈렸다:** 응답 매핑 `2/1`(추가 0), 제출값 수용 `2/2`(추가 0), 사후 재조회 `3/1`(**추가 1회**). `refetch`는 `WRITE-1` 종료와 **같은 시각** `READ-3`이 시작하고, 그 READ를 완료하기 전까지 `mutation phase`가 `pending`으로 남는다 — WRITE가 끝나도 작업은 끝나지 않는다.
- **B8-7-04를 해소했다([DC8-5-34·36](./PHASE8_5.md)):** 데모가 수용 4가지 중 `submitted` 하나만 쓰고 mock WRITE가 받은 값을 그대로 저장해, **서버 보정 상황 자체가 만들어지지 않았다.** 서버 보정 WRITE(`success-corrected`, 우편번호를 5자리로 정규화)와 저장 둘(`save-with-response`·`save-with-refetch`)을 더했다. 응답에는 **저장된 레코드 전체**를 실었다 — 일부 필드만 주면 앱이 나머지를 지역 편집값으로 메우게 되고 그것은 B8-7-03의 반복이다.
- **기존 테스트가 새 조작의 결함을 즉시 잡았다([DC8-5-35](./PHASE8_5.md)):** 저장 조작이 둘이 되자 카탈로그 테스트가 실패했다 — 같은 query에 연결 작업을 두 개 시작하면 sync가 거절하는데 데모가 그것을 던져 버렸다. 세 저장 조작 모두 `pending`을 먼저 보고 말로 답한다.
- M2-07 둘째 항목의 `revision` 문구는 [R2-09](./REQUIREMENTS.md)에 없는 부연이고 이 fixture의 `Profile`에 revision 필드가 없어 닫았다([DC8-5-37](./PHASE8_5.md)). 라이브러리 공백이 아니며, 네 수용 방식의 런타임 계약은 `packages/sync/src/tests/`가 모두 덮는다.
- 조작은 38개에서 **41개**, `examples/shared` 테스트는 26개에서 **32개**가 되었다. 다른 수치는 불변이고 `pnpm gate` 19단계·`pnpm check:examples` 모두 PASS다. **`packages/` 소스 변경 0.**
- next: M2-09(실패한 작업만 복구)부터 이어간다. `다음 WRITE 확정 거절 예약`과 `다음 WRITE 결과 불명 예약`이 거기서 처음 쓰인다. M2-04의 남은 둘은 Vue SSR 데모(5192)와 렌더 계측 차례다.
- blockers: 없다. B8-7-01~04 모두 해소했다.
- 시작 기준 commit: `5a95a7c`.
...
- **이 표는 기능 동등성 선언이 아니다.** 9행 전부의 근거가 Node에서 도는 자동 테스트이며, 브라우저 증거는 없다.
- next: Phase 8.7 — 사람이 브라우저에서 M2-01~20을 수행한다. 이 단계만 자동화하지 않는다.
