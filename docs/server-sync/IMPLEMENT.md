...
- [ ] 현재 Node/pnpm/TS와 core·커넥터의 테스트·타입·빌드·bench·bundle baseline을 기록한다.
- [ ] IC2-01: ref 입력 draft, metadata 위치, 공개 API 이름·타입, readonly와 loaded guard를 검증한다.
- [ ] IC2-03: 독립 엔진의 F2 세부 목록·기준 버전·기본값·완료 조건과 단계별 배포 범위를 기록한다.
- [ ] IC2-02/05: 두 비교 기준, 변경 기록, 동기 local apply·재진입·배열 경계의 최소 모델을 검증한다.
- [ ] IC2-04: 임의 DTO와 제출 기록·서버 보정·후속 입력 연결의 최소 모델을 만든다.
- [ ] IC2-06: hydration/영속화에 포함할 기준·편집·작업과 플랫폼 수명 설계를 시작한다.
...
| 기능 동등성 F2 세부 검증 | 미수행 — 기준 버전·상세 목록 조사 필요 |
