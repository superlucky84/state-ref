...
상태: 2026-09-21 Phase 4 mutation 이후 Phase 5.1의 깨끗한 서버 기준 SSR 전달, Phase 5.2의 초기 기준·캐시 준비, Phase 5.3의 관찰자별 view, Phase 5.4의 자동 enabled/key 전환, Phase 5.5의 5종 UI 읽기 전용 view 연결, Phase 5.6의 client별 focus/reconnect/polling 하위 범위까지 구현했다. **Phase 5의 나머지 기능과 Phase 6 resource/draft 조합은 다음 작업**이다. state-ref 코어·서버 동기화 헬퍼·draft 헬퍼를 선택적으로 조합한다. 서버 기능은 계속 추진하며, TanStack Query의 query/mutation 모델과 기능을 참고하되 런타임 독립을 지향한다.
