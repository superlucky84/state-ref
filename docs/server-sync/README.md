...
상태: 2026-09-22 Phase 4 mutation 이후 Phase 5.1~5.13의 조회·view·UI 연결·영속화·연결 제출·읽기 전용 캐시 관측·무한 조회 편의 API 하위 범위까지 구현했다. **Phase 5의 나머지 기능과 Phase 6 resource/draft 조합은 다음 작업**이다. state-ref 코어·서버 동기화 헬퍼·draft 헬퍼를 선택적으로 조합한다. 서버 기능은 계속 추진하며, TanStack Query의 query/mutation 모델과 기능을 참고하되 런타임 독립을 지향한다.
...
`client.liveView(source, resolve)`는 `state-ref` 입력의 enabled/key 변경을 따라가며 READ를 자동 시작한다. 안정된 표시 ref에서 이전 key 값을 즉시 제거하고, 마지막 소유자가 떠난 READ의 늦은 결과를 기준에 반영하지 않는다. 동일 key의 다른 소유자는 공유 READ를 유지한다. 이 단계의 런타임 계약과 증거는 [Phase 5.4 기록](./PHASE5_4.md)에 있다.

## Phase 5.5 진행 — 5종 UI 읽기 전용 View

`connectReactView`·`connectPreactView`·`connectVueView`·`connectSvelteView`·`connectSolidView`가 sync의 읽기 전용 `view.watch`를 한 방향으로 표시한다. 실제 프레임워크 테스트에서 key 전환, 늦은 결과 차단, 로컬 resource 편집, 언마운트 후 구독 정리를 확인했다. 커넥터는 sync 런타임을 import하지 않고, view의 소유자는 별도로 `dispose()`한다. 전체 resource/draft/pending UI 조합과 수동 M2는 [Phase 5.5 기록](./PHASE5_5.md)에 남은 범위로 명시한다.

## Phase 5.6 진행 — Focus·Reconnect·Polling 자동 재조회

`createSyncClient({ environment })`는 client에 주입한 focus/reconnect 사건과 현재 focused/online 상태만 사용한다. 한 번 load/refetch한 활성 query와 자동 load하는 `liveView`가 관찰자별 stale/always/disabled 정책에 참여한다. polling은 양의 유한 interval로 opt-in하고 offline과 기본 background 상태에서 멈춘다. 같은 key 요청 공유, linked WRITE 차단, 로컬 편집 rebase, 실패 복구, dispose/SSR 정리를 자동 검증했다. 브라우저 adapter 제품화, network mode와 나머지 F2 범위는 [Phase 5.6 기록](./PHASE5_6.md)에 명시한다.

## Phase 5.7 진행 — Pagination·Infinite Query
...
query별 `online`·`always`·`offlineFirst`는 오프라인 READ 시작과 retry 대기, reconnect 재개를 구분한다. `createBrowserSyncEnvironment()`는 명시적 호출 시 브라우저 focus/visibility/online 신호를 client별 환경으로 연결한다. 이 조회 계약은 [Phase 5.8 기록](./PHASE5_8.md)에 있다.

## Phase 5.9 진행 — Clean 기준 영속화·독립 명령 Queue
...
## Phase 5.11 진행 — 연결 제출 기록과 Durable 전송 장벽

`openPersistedLinkedMutation`은 한 storage key에 연결 제출 1건의 JSON DTO·선택한 변경·idempotency key와 로컬 snapshot을 함께 저장한다. 명시적 `send` 전에 현재 편집을 재검사하고, WRITE 직전에 `inFlight`와 보수적인 미확정 snapshot을 저장한다. 재시작한 `inFlight`는 `unknown`으로 보류하며 자동 재전송하지 않는다. 결과와 후속 로컬 상태는 가능할 때 저장한다. 다중 연결·함수형 응답 매핑·진행 중 편집의 연속 저장은 지원하지 않는다. 범위와 반례는 [Phase 5.11 기록](./PHASE5_11.md)에 있다.

## Phase 5.12 진행 — Client 캐시 관측 경계

`client.inspectCache()`와 `subscribeCache()`는 client별 query key·kind·소유자 수·상태를 읽기 전용으로 제공한다. 생성·변경·제거 이벤트는 해당 시점의 metadata를 microtask에서 순서대로 전달하며, 구독자 오류와 해제는 query 실행에 영향을 주지 않는다. payload와 mutation DTO는 자동 노출하지 않는다. 범위와 반례는 [Phase 5.12 기록](./PHASE5_12.md)에 있다.

## Phase 5.13 진행 — 무한 조회 캐시 준비와 관찰자 View

`client.fetchInfinite()`/`prefetchInfinite()`/`ensureInfinite()`은 무한 조회 cache를 임시 소유권으로 준비한다. 활성 조회 설정을 교체하지 않고, 같은 key의 READ를 공유한다. `client.infiniteView()`는 query의 양방향 페이지 메서드와 observer별 placeholder/select 표시를 함께 제공한다. 범위와 반례는 [Phase 5.13 기록](./PHASE5_13.md)에 있다.
