...
5. **8.5 — `examples/` 데모와 gate 편입.** DC8-02의 데모에 브라우저 hydration과 loading/error 화면을 포함하고 문서 예제를 타입 검사에 넣는다. **기준 테스트:** core-only·draft-only·sync-only·전체 조합 번들이 각각 동작하고 의존성 경계가 확인될 것, 문서 예제가 공개 선언 타입으로 컴파일될 것. (M2-01, M2-02, M2-04의 준비)
