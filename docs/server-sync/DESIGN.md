...
- [ ] **IC2-06 / Phase 0~5** hydration/영속화 시 서버 기준과 로컬 변경·진행 작업을 구별하는 저장 형식, 개발 도구와 플랫폼 통합을 설계한다. [Phase 5.1](./PHASE5_1.md)에서 clean baseline SSR 전달, [Phase 5.6](./PHASE5_6.md)에서 전역 상태 없는 client별 환경 사건 계약과 focus/reconnect/polling 수명을 구현했다. 브라우저 adapter 제품화, 영속화·오프라인·개발 도구와 로컬 변경 복원은 미완료다.
