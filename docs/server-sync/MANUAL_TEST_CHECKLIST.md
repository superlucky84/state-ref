...
**다섯 종으로 넓힘 — e2e 실행으로 채움**(DC8-8-08). 구현 SHA `0a08c1c`, 시나리오 `M2-07-response`·`M2-07-submitted`·`M2-07-refetch`, 명령 `pnpm test:e2e`. 세 수용 방식의 READ 비용과 기준값의 출처가 **Preact·Vue·Svelte·Solid에서도 같은 단계마다 같은 판독**으로 나온다. `refetch`에서 **WRITE가 끝나고 서버가 이미 `부산 / 2`인데도 작업이 `pending`에 머무는** 한 단계를 따로 고정했다 — 기준이 아직 읽지 않은 READ에서 오기 때문이다. 시나리오는 각각 새 모델에서 시작하므로 위 표의 `submitted` 행 `2 / 2`(한 세션 누적)와 달리 `2 / 1`로 적혀 있다.
...
**다섯 종으로 넓힘 — e2e 실행으로 채움**(DC8-8-08). 구현 SHA `688db69`, 시나리오 `M2-09-keep`·`M2-09-remove`·`M2-09-later-input`·`M2-09-server-update`·`M2-09-earlier-success`, 명령 `pnpm test:e2e`. 아래 표의 두 정책과, 산문으로만 적혀 있던 세 흐름(후속 입력의 재기준화, 무관한 서버 갱신의 생존, 먼저 성공한 기준을 지우지 않음)이 **Preact·Vue·Svelte·Solid에서도 같은 단계마다 같은 판독**으로 나온다. 셋째 항목은 데모 한계로 그대로 닫혀 있다.
...
- [x] `liveView`를 비활성 상태에서 활성화하고, 진행 READ 중 key를 바꾼다. 이전 key의 늦은 결과가 새 표시·기준에 들어가지 않고 마지막 소유자일 때 signal이 취소되는지 확인한다.
- [ ] 같은 이전 key를 다른 view가 보고 있다면 공유 READ는 유지하고 그 view에서만 결과를 확인한다. 비활성화와 화면 해제 뒤에는 표시와 구독이 남지 않는지 확인한다. — **부분.** 비활성화·해제 뒤 **표시**가 남지 않는 것은 확인했다(해제 뒤 모든 행이 `(해제됨)`). 나머지 둘은 도달 불가: 같은 key를 보는 **둘째 view**가 데모에 없고, **구독이 남지 않는지**는 화면에 구독 수 계측이 없다(M2-18과 같은 계측을 요구한다).
- [x] 연결되지 않은 query까지 자동 보호한다고 표시하지 않는다.

**합격:** 알고 있는 연결 범위에서 약속한 순서를 보장함. **결과: 항목 1·2·4·6 통과, 항목 5 부분, 항목 3 미수행.**

**항목 4 — e2e 실행으로 채움**(DC8-8-08). 구현 SHA `b235596`, 2026-09-27, 시나리오 `M2-11-4-activate`·`M2-11-4-cancel`·`M2-11-4-late`, 명령 `pnpm test:e2e`, **데모 다섯 종 전부.** 데모에 `liveView` 카드를 만들어 닿았다 — 라이브러리에는 Phase 5부터 있었고 데모에만 없었다. 두 key(`live/a`·`live/b`)가 **서로 다른 값**(`서울-a`·`서울-b`)을 답하도록 mock을 거쳐 태깅했다. 같은 값을 답하면 "이전 key의 늦은 결과가 새 표시에 들어가지 않았다"가 보이지 않는다([DC8-5-48](./PHASE8_5.md)과 같은 함정).

- **비활성 → 활성 → 비활성 → 해제.** 원본이 key를 가리키지 않으면 `enabled=false`·조회 없음이다. `key live/a로 활성화`가 조회를 시작하고 표시가 `서울-a`를 든다. `표시 비활성화`는 표시를 비우고 **요청을 늘리지 않는다**. `표시 해제` 뒤에는 모든 행이 `(해제됨)`이다.
- **key 전환이 마지막 소유자의 조회를 취소한다.** `live/a`의 READ가 떠 있는 동안 `live/b`로 바꾸면 `READ-1 live/a`가 **`aborted`**가 되고 `READ-2 live/b`가 시작된다.
- **signal을 무시한 이전 key의 늦은 결과는 표시에 들어가지 않는다.** `다음 READ는 signal을 무시함`을 걸고 같은 전환을 하면 `READ-1`이 취소되지 않고 남는다. 그것을 먼저 완료시키면 `READ-1 success`가 되지만 표시는 여전히 `live/b`를 기다리는 `pending / fetching`·`(없음)`이고 **`서울-a`는 어디에도 나타나지 않는다.** 이어서 `READ-2`를 완료하면 `서울-b`만 들어온다.
...
- [x] draft가 수정하지 않은 원본 필드의 갱신은 draft에도 보인다.
- [x] draft 도시 대전과 원본 도시 광주가 겹치면 기준 부산·내 입력 대전·원본 광주를 확인한다.
- [x] 충돌을 해결하기 전 조용한 덮어쓰기나 입력 손실이 없다.
- [x] 원본도 대전으로 수렴하는 경우와 원본의 낙관적 값이 복구되는 경우를 별도로 확인한다.

**합격:** 원본의 최신 변경과 독립 입력을 함께 유지함. **결과: 통과.**

**e2e 실행으로 채움**(DC8-8-08). 구현 SHA `52ac8c6`, 2026-09-26, 시나리오 `M2-13-unrelated`·`-conflict`·`-resolve-source`·`-resolve-draft`·`-source-rollback`·`-converge`, 명령 `pnpm test:e2e`, **데모 다섯 종 전부.** 처음 수행한 항목이고, 수행하려면 계측 2종을 먼저 더해야 했으며 수행 중에 데모 결함 1종을 찾았다(아래 셋).

- **원본의 무관한 필드 갱신이 draft에 보인다.** draft는 city만 편집했고, 원본이 `memo`를 바꾸자 draft 카드의 `memo`도 같이 바뀌었다. draft의 변경 목록은 `city` 한 줄 그대로다 — **보이는 값과 내 변경은 다른 축이다.**
- **겹치면 한 줄이 세 값을 말한다.** 원본을 `광주`로 바꾸자 draft의 변경이 `city "부산" → "대전" 원본 "광주" conflict`가 되었다. 기준은 갈라져 나온 `부산`, 내 입력은 `대전`, 원본은 `광주`다. draft는 여전히 `대전`을 들고 있어 **조용한 덮어쓰기도 입력 손실도 없다.** 충돌 수 1, `READ/WRITE = 2 / 0`.
- **해소는 두 방향 모두 확인했다.** `원본 선택`은 draft를 `광주`·clean으로 만들고, `draft 선택`은 `대전`을 지키면서 그 변경의 기준을 `광주`로 옮겨 붙여 충돌을 없앤다 — **같은 입력이 새 기준 위에서 다시 서술된다.**
- **수렴과 낙관적 값 복구를 따로 확인했다.** `draft A 로컬 적용`은 원본을 `대전`으로 수렴시키고 draft를 clean으로 만든다(요청 0회). 반대로 원본의 제출 입력이 거절로 되돌려져 원본이 `서울`로 갈 때, draft는 두 번의 원본 변경과 한 번의 거절을 지나고도 `대전`을 지키며 충돌을 **현재** 원본 값(`서울`)에 대해 보고한다.
- **관찰:** 로컬 적용은 원본의 변경을 `(root)` 한 줄로 기록한다 — draft가 객체 전체를 쓰기 때문이다. 그래서 표가 `city` 한 줄이 아니라 레코드 전체를 담은 한 줄이 된다. 결함이 아니라 적용 경로의 성질로 기록한다(M2-14가 같은 자리를 더 본다).

<a id="b8-7-16"></a>
**B8-7-16 — draft 충돌이 세 값 중 둘만 보여 줬다. 해소됨.** `draftChangeLines`가 `DraftChange.source`를 버려서, 화면에는 기준과 내 입력만 있고 **무엇과 충돌하는지가 없었다.** 이 항목의 둘째 문장이 요구하는 것이 정확히 그 세 값이다. draft 표에만 `원본` 열이 생기고(resource 표는 불변) 판독기도 그 칸을 읽는다.

<a id="b8-7-17"></a>
**B8-7-17 — draft 카드에 draft가 건드리지 않는 필드가 없었다. 해소됨.** draft 카드는 `도시`와 `우편번호`만 그렸고 둘 다 draft가 편집하는 대상이다. 그래서 "원본의 갱신이 draft에도 보인다"를 볼 대상이 화면에 없었다. `메모` 행을 더했다.

<a id="b8-7-18"></a>
**B8-7-18 — Preact 데모의 draft 카드가 상태를 구독하지 않았다. 해소됨.** `draftPanel(draft.status.value, …)`로 **구독 없이** 상태를 읽어, draft의 *값*이 바뀔 때만 다시 그려졌다. 원본이 바뀌면 draft의 충돌 수와 version은 움직이지만 값은 그대로이므로, 카드가 **낡은 충돌 수와 낡은 원본 값에 앉아 있었다.** React는 `draft.watchStatus`를 구독한다. **모델 판독은 통과하는데 다섯 중 Preact만 실패했다** — [DC8-5-01](./PHASE8_5.md)이 전제한 "화면이 갈리면 커넥터 구간 탓"의 첫 실제 사례이고, 이 장치 없이는 볼 수 없었다.

### M2-14 — Draft 적용 후 두 changes (R2-17/18)

- [x] 서버 서울 → resource 부산 → draft 대전 상태를 준비한다.
- [x] 원본의 무관한 필드도 바꾼 후 draft를 apply한다. 그 변경은 유지된다.
- [x] draft는 대전/clean, resource는 대전/서울 → 대전/dirty다. — **단, 원본의 변경은 `city` 한 줄이 아니라 `(root)` 한 줄로 기록된다.** 아래 관찰 참고.
- [x] apply 자체의 READ/WRITE는 0회다. 별도 mutation 성공·기준 수용 후 resource 변경이 해소된다.
- [x] 충돌 시 전체 적용을 무변경 거절하고, 적용 중 추가 입력은 이전 적용의 완료 처리로 지우지 않는다.

**합격:** 로컬 반영과 서버 저장, 두 변경 기준이 일관됨. **결과: 통과.**

**e2e 실행으로 채움**(DC8-8-08). 구현 SHA `4bf156e`, 2026-09-27, 시나리오 `M2-14-apply`·`M2-14-refuse`·`M2-14-later-edit`, 명령 `pnpm test:e2e`, **데모 다섯 종 전부.** 처음 수행한 항목이고, 수행 중 데모 결함 1종을 찾아 고쳤다([B8-7-19](#b8-7-19)).

- **적용은 요청 없이 원본을 바꾸고 무관한 변경을 지우지 않는다.** `draft A 로컬 적용` 뒤 draft는 `대전`·clean, 원본은 `대전`·dirty이고 `READ/WRITE = 2 / 0`이다. 적용 직전에 바꾼 원본의 `memo`는 그대로 남아 `(root)` 줄 안에 함께 실린다.
- **관찰 — 원본의 변경이 `(root)` 한 줄이다.** `draft.apply()`가 레코드 전체를 한 번에 쓰기 때문이다(`packages/state-ref/src/draft/index.ts:398`). 그래서 표에 `city "서울" → "대전"` 대신 **레코드 전체를 담은 한 줄**이 선다. 체크리스트 셋째 문장이 기대하는 정보는 그 줄 안에 다 있지만 읽기에는 불편하다. **라이브러리의 원자적 쓰기 성질이고 결함이 아니므로** 화면을 바꾸지 않고 이렇게 기록한다.
- **적용한 변경이 저장으로 해소된다.** `제출할 변경 고정`(변경 1건) → `저장 실행 (제출값 수용)` → 완료 뒤 원본이 `clean`·`변경 없음`이고 서버는 `대전 / 2`, `READ/WRITE = 2 / 1`이다.
- **충돌하면 전체를 무변경으로 거절한다.** 원본을 `광주`로 바꿔 draft와 겹치게 한 뒤 적용하면 `적용 거절: conflict`이고 **원본은 전혀 움직이지 않는다** — `(root)` 줄이 생기지 않고 `city` 한 줄과 version이 그대로다. 부분 적용이 없다.
- **적용 뒤의 새 입력이 살아남는다.** 적용 → 원본을 `광주`로 → draft를 다시 `대전`으로 하면 그 입력이 새 기준(`광주`) 위의 변경으로 남는다. 앞선 적용이 정리한 편집 목록에 휩쓸리지 않는다.

<a id="b8-7-19"></a>
**B8-7-19 — 로컬 적용이 남긴 변경을 영원히 제출할 수 없었다. 해소됨.** 데모의 `제출할 변경 고정`이 `change.path[0]`으로 DTO 경로를 걸렀는데, 적용이 남긴 변경은 **경로가 루트라 `path[0]`이 없다.** 그래서 고정이 **변경 0건**을 골라 저장이 아무것도 보내지 않았고, 원본은 **보낼 수 없는 변경을 안은 채 영구히 dirty**였다 — 이 항목의 넷째 문장이 성립하지 않는 상태였다. 루트 변경은 레코드 전체를 담으므로 DTO가 싣는 경로가 그 안에 있고 제출 가능하다. 필터가 루트 변경을 함께 고르게 했고, 결과 문구가 그 사실을 말한다. **라이브러리가 아니라 데모의 가정(모든 변경에 leaf 경로가 있다)이 틀렸다.**

### M2-15 — Reset·Discard와 원본 보존 (R2-19)

- [x] resource 부산, draft 대전 상태에서 reset하면 draft는 현재 원본 부산을 따르고 세션은 유지된다.
- [x] 별도 실행에서 discard하면 draft는 종료되고 resource의 서울 → 부산은 남는다.
- [x] 이미 apply한 내용을 draft 폐기로 되돌리지 않는다.
- [x] 종료된 ref 사용은 명시적으로 실패하고, 네트워크 취소나 저장 취소로 표시하지 않는다.

**합격:** 초기화·폐기는 자기 draft의 입력과 수명에만 작용함. **결과: 통과.**

**e2e 실행으로 채움**(DC8-8-08). 구현 SHA `566b2ab`(항목 1~3)·`98c13a9`(항목 4), 2026-09-27, 시나리오 `M2-15-reset`·`M2-15-discard`·`M2-15-discard-after-apply`, 명령 `pnpm test:e2e`, **데모 다섯 종 전부.** 처음 수행한 항목이다.

- **reset은 입력만 지우고 세션을 끝내지 않는다.** draft A가 현재 원본 값(`부산`)을 따라 clean이 되고, 원본의 `서울 → 부산`과 draft B는 그대로다. **바로 뒤에 같은 draft에 다시 입력이 들어간다** — 그것이 reset과 discard를 가르는 지점이다.
- **discard는 그 draft만 끝낸다.** draft A 카드가 화면에서 사라지고 원본의 변경과 draft B는 남는다.
- **적용한 내용은 폐기로 되돌아가지 않는다.** 적용 뒤 폐기해도 원본은 `대전`(`(root)` 줄)이고 draft B도 원본을 따라 `대전`이다.
- **종료된 ref가 명시적으로 거절하고, 그 거절이 네트워크·저장 취소로 보이지 않는다.** 폐기 뒤 `draft A 도시 → 대전`은 데모 자신의 가드가 먼저 답한다(`먼저 draft를 분기한다.`) — 그것은 아직 ref의 거절이 아니다. 조작 `폐기한 draft A에 쓰기 시도`가 붙잡아 둔 종료된 ref에 일부러 쓰고, **`Error: This draft has been discarded.`**로 거절된다. 그 동안 원본은 `부산` 그대로, `mutation phase = idle`, `진행 중 WRITE 0`, `READ/WRITE = 2 / 0`이다 — 네트워크 취소도 저장 취소도 아니다. 이 조작을 위해 카탈로그가 47개가 되었고, `readonly 조회에 쓰기 시도`와 같은 계열이다(금지된 것을 시도하고 돌아온 말을 그대로 보여 준다).
