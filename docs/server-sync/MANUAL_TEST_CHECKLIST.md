...
- [ ] 종료된 ref 사용은 명시적으로 실패하고, 네트워크 취소나 저장 취소로 표시하지 않는다. — **후반부만 확인.** 폐기 뒤 draft 조작은 draft의 어휘로 거절되고 `mutation phase`·`READ/WRITE`가 움직이지 않는다(네트워크 취소·저장 취소로 표시되지 않음). 전반부(**종료된 ref 자체가 명시적으로 실패한다**)는 데모가 죽은 ref를 건드리지 않아 도달하지 못한다 — 폐기 뒤 슬롯이 비어 데모 자신의 가드가 먼저 답한다. `readonly 조회에 쓰기 시도`와 같은 계열의 조작을 하나 더하면 닿는다.

**합격:** 초기화·폐기는 자기 draft의 입력과 수명에만 작용함. **결과: 항목 1·2·3 통과, 항목 4 부분(후반부만).**

**e2e 실행으로 채움**(DC8-8-08). 구현 SHA `566b2ab`, 2026-09-27, 시나리오 `M2-15-reset`·`M2-15-discard`·`M2-15-discard-after-apply`, 명령 `pnpm test:e2e`, **데모 다섯 종 전부.** 처음 수행한 항목이다.
...
- **거절이 네트워크·저장 취소로 표시되지 않는다.** 폐기 뒤 `draft A 도시 → 대전`을 누르면 `먼저 draft를 분기한다.`로 답하고 `mutation phase = idle`·`진행 중 WRITE 0`·`READ/WRITE = 2 / 0`이 그대로다.
